...
async function startSecureMatch(body={}) {
  ...
  if(tournamentMode){
    ...
    if(progress.localFallback!==true && (progress.completed===true || progress.won===true || !String(progress.cloudRunToken||''))){
      ...
      if(!renewed.ok)return {_status:renewed.status||503,code:renewed.error||'TOURNAMENT_ENTRY_REJECTED'};
    }

    const tournamentClubPreflight=await prepareOfflineMatchClubSync('before-tournament-match');
    if(!tournamentClubPreflight.ok){
      return {_status:503,code:'CLUB_SYNC_FAILED',reason:tournamentClubPreflight.error||'CLUB_SYNC_FAILED'};
    }
    ...
    if(!authorization.ok){
      ...
      if(Number(authorization.status)===429 && authError==='MATCH_EARLY_FORFEIT_LIMIT'){
        progress.localFallback=true;
        progress.localFallbackReason='MATCH_EARLY_FORFEIT_LIMIT';
        progress.updatedAt=Date.now();

        active.cloudToken='';
        active.cloudAuthorized=false;
        active.cloudFallback=true;
        active.cloudFallbackReason='MATCH_EARLY_FORFEIT_LIMIT';
        saveState();

        logger(`[mng-match-session] V116 early-forfeit limiter bypass matchId=${local.matchId} tournamentId=${tournamentId}; continuing LOCAL`);
        return {...local,cloudAuthorized:false,localFallback:true,tournamentId};
      }

      if(Number(authorization.status)!==409){
        clearSecureSeasonActiveMatch(local.matchId);
      }
      saveState();
      logger(`[mng-match-session] V111 tournament authorization failed matchId=${local.matchId} tournamentId=${tournamentId} status=${authorization.status} error=${authorization.error||''}`);
      return {_status:authorization.status||503,code:authorization.error||'MATCH_AUTHORIZATION_FAILED'};
    }
  ...
  if(!seasonClubPreflight.ok){
    return {_status:503,code:'CLUB_SYNC_FAILED',reason:seasonClubPreflight.error||'CLUB_SYNC_FAILED'};
  }
  ...
  if(!authorization.ok){
    if(Number(authorization.status)===409){
      // Normal Solo Seasons have a complete local reward/progression engine.
      // Do not kick the user out of FUT merely because the old cloud match
      // service still has a stale start session.
      active.cloudFallback=true;
      active.cloudFallbackReason=String(authorization.error||'MATCH_START_CONFLICT');
      active.cloudAuthorized=false;
      saveState();
      logger(`[mng-match-session] V111 season local fallback matchId=${local.matchId} status=409 error=${authorization.error||''}`);
