...
async function startSecureMatch(body={}) {
  ...
  if(tournamentMode){
    ...
    if(!await forceMngCloudClubSync('before-tournament-match')){
      return {_status:503,code:'CLUB_SYNC_FAILED'};
    }
  ...
  if(!await forceMngCloudClubSync('before-season-match')){
    return {_status:503,code:'CLUB_SYNC_FAILED'};
  }
...
async function handle(req,res,urlPath,requestBody) {
  ...
  if(!state.__diagHotfixSeen){
    ...
    logger('[diag] V111 match-launch sessions active - idempotent Solo/Season + fresh Solo Cup cloud runs + full cloud save');
  }
