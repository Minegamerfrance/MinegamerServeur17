...
async function startSecureMatch(body={}) {
  ...
  if(tournamentMode){
    ...
    const activeBeforeAuth=secureSeasonActiveMatch(local,body,'tournament',tournamentId);
    if(!activeBeforeAuth)return {_status:500,code:'MATCH_STATE_CREATE_FAILED'};
    ...
    if(!activeAfterAuth){
      logger(`[mng-match] V100 tournament activeMatch missing after authorization matchId=${local.matchId}`);
      return {_status:500,code:'MATCH_STATE_LOST'};
    }
    ...
    logger(`[mng-match] V100 cloud tournament start authorized matchId=${local.matchId} tournamentId=${tournamentId} token=${String(authorization.token||'').slice(0,8)}`);
  ...
  const activeBeforeAuth=secureSeasonActiveMatch(local,body,'season',0);
  if(!activeBeforeAuth)return {_status:500,code:'MATCH_STATE_CREATE_FAILED'};
  ...
  if(!activeAfterAuth){
    logger(`[mng-match] V100 season activeMatch missing after authorization matchId=${local.matchId}`);
    return {_status:500,code:'MATCH_STATE_LOST'};
  }
  ...
  logger(`[mng-match] V100 cloud season start authorized matchId=${local.matchId} token=${String(authorization.token||'').slice(0,8)}`);
  return {...local,cloudAuthorized:true};
}
...
async function handle(req,res,urlPath,requestBody) {
  ...
  if(!state.__diagHotfixSeen){
    ...
    logger('[diag] V110 full-account cloud save active - club/items/squads/SBC/draft/seasons/tournaments + wallet');
  }
