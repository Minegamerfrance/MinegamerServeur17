...
function offlineMatchReadyDocument(body={}) {
  ...
  if(!active||active.completed===true||Number(active.id)<=0){
    logger('[mng-match-ready] V113 rejected: no active offline match');
  ...
  const squad=squadDocument(Number(active.squadId)||Number(state.activeSquadId)||1);
  const startDateTime=Math.floor(Math.max(0,Number(active.createdAt)||Date.now())/1000);

  logger(`[mng-match-ready] V113 matchId=${Number(active.id)||0} mode=${active.mode||'season'} tournamentId=${Number(active.tournamentId)||0} starters=${active.matchReadyItemIds.length}`);

  // Retail/native MatchReady shape. Do NOT return the CreateMatch document here.
  return {
    squad,
    startDateTime
  };
...
async function startSecureMatch(body={}) {
  ...
  if(draftMode){
    ...
    const authorization=await cloudMatchRequest('/api/matches/start',{
      mode:'draft',clientMatchId:Number(local.matchId),round:Number(local.round)||1,
      divisionId:10,draftRunToken:String(state.offlineDraft?.cloudRunToken||'')
    });
    if(!authorization.ok){
    ...
    state.offlineDraft.activeMatch.cloudToken=String(authorization.token||'');
    saveState();
...
async function handle(req,res,urlPath,requestBody) {
  ...
  if(!state.__diagHotfixSeen){
    ...
    logger('[diag] V114 tournament native-state guard active - cloud-only runs hidden until bracket/progress data exists');
  }
