...
# MNG-BRIDGE-OWNS-FIFA-2.1.34
# Legacy handoff is permanently disabled; the Bridge/server always starts FIFA.
...
$persistentDataRoot=Join-Path $persistentStateRoot 'server-data'
$sessionDataRoot=Join-Path $root 'data'
New-Item -ItemType Directory -Path $persistentDataRoot -Force | Out-Null
New-Item -ItemType Directory -Path $sessionDataRoot -Force | Out-Null
foreach($name in @('mng-cloud-session.json','mng-cloud-club.json')){
    $persistentFile=Join-Path $persistentStateRoot $name
    if(Test-Path -LiteralPath $persistentFile -PathType Leaf){
        Copy-Item -LiteralPath $persistentFile -Destination (Join-Path $sessionDataRoot $name) -Force
    }
}
$persistentProfiles=Join-Path $persistentDataRoot 'profiles'
if(-not (Test-Path -LiteralPath $persistentProfiles -PathType Container)){
    $legacyProfileRoots=@(
        (Join-Path $persistentStateRoot 'server\data\profiles'),
        (Join-Path $persistentStateRoot 'runtime\MNGFUT\data\profiles')
    )
    foreach($legacyProfiles in $legacyProfileRoots){
        if(Test-Path -LiteralPath $legacyProfiles -PathType Container){
            Copy-Item -LiteralPath $legacyProfiles -Destination $persistentDataRoot -Recurse -Force
            Write-Host ("MNG FUT SAVE: anciens profils recuperes depuis {0}" -f $legacyProfiles)
            break
        }
    }
}
if(Test-Path -LiteralPath $persistentProfiles -PathType Container){
    Copy-Item -LiteralPath $persistentProfiles -Destination $sessionDataRoot -Recurse -Force
}
...
$existingFifa=@(Get-Process -Name FIFA17,stp-fifa17 -ErrorAction SilentlyContinue)
if($existingFifa){
    Write-Host 'FIFA 17 est deja ouvert. Fermeture automatique de l ancienne instance...' -ForegroundColor Yellow
    foreach($process in $existingFifa){
        Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue
    }
    $closeDeadline=(Get-Date).AddSeconds(12)
    do {
        Start-Sleep -Milliseconds 250
        $remainingFifa=@(Get-Process -Name FIFA17,stp-fifa17 -ErrorAction SilentlyContinue)
    } while($remainingFifa -and (Get-Date) -lt $closeDeadline)
    if($remainingFifa){
        throw 'FIFA 17 n a pas pu etre ferme automatiquement. Ferme FIFA17.exe dans le Gestionnaire des taches puis relance MNG FUT.'
    }
    Write-Host 'Ancienne instance FIFA 17 fermee.' -ForegroundColor Green
}
New-Item -ItemType Directory -Force -Path (Join-Path $root 'logs') | Out-Null
@(
...
$useModData=$false
if($env:MNG_FUT_SERVER_ONLY -eq '1'){
...
function Ensure-MngDnsClient {
    param([string]$LogPath)

    $service=Get-Service -Name 'Dnscache' -ErrorAction Stop
    if($service.Status -ne 'Running'){
        Add-Content -LiteralPath $LogPath -Value "DNS Client service status before repair: $($service.Status)"
        try { Set-Service -Name 'Dnscache' -StartupType Automatic -ErrorAction Stop } catch {
            Add-Content -LiteralPath $LogPath -Value "DNS Client startup type warning: $($_.Exception.Message)"
        }
        Start-Service -Name 'Dnscache' -ErrorAction Stop
        $service.WaitForStatus([System.ServiceProcess.ServiceControllerStatus]::Running,[TimeSpan]::FromSeconds(8))
    }
    Add-Content -LiteralPath $LogPath -Value "DNS Client service status: $((Get-Service -Name 'Dnscache').Status)"
}

function Assert-MngLocalSenorClutch {
    param([string]$LogPath,[string]$HostsPath)

    $hostsText=[IO.File]::ReadAllText($HostsPath)
    $localMapping=@($hostsText -split "`r?`n" | Where-Object {
        $_ -match '^\s*127\.0\.0\.1\s+senorclutch\.game(?:\s|$)' -and
        (Test-MngHostsLineForName -Line $_ -Name 'senorclutch.game')
    })
    if($localMapping.Count -eq 0){
        throw 'La redirection locale senorclutch.game est absente du fichier hosts.'
    }

    Add-Content -LiteralPath $LogPath -Value 'MNG DNS V53: hosts mapping verified directly; Windows .NET DNS cache check disabled.'
    Write-Host 'MNG DNS V53: redirection locale senorclutch.game valide.' -ForegroundColor Green
}
...
$persistentDiagnostics=Join-Path $persistentStateRoot 'diagnostics\fifa17-latest'
New-Item -ItemType Directory -Force -Path $persistentDiagnostics | Out-Null
Get-ChildItem -LiteralPath $persistentDiagnostics -File -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue

function Save-MngPersistentDiagnostics {
    param([Diagnostics.Process]$ServerProcess)

    Copy-Item -Path (Join-Path $logDir '*') -Destination $persistentDiagnostics -Force -ErrorAction SilentlyContinue
    $senorLog=Join-Path $gameDir 'senorclutch-log.txt'
    if(Test-Path -LiteralPath $senorLog){Copy-Item -LiteralPath $senorLog -Destination (Join-Path $persistentDiagnostics 'senorclutch-log.txt') -Force -ErrorAction SilentlyContinue}
    Copy-Item -LiteralPath $hosts -Destination (Join-Path $persistentDiagnostics 'hosts-current.txt') -Force -ErrorAction SilentlyContinue
    $serverPid=if($ServerProcess){$ServerProcess.Id}else{0}
    $listeners=@(
        Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue|Where-Object{$tcpPorts -contains $_.LocalPort}|ForEach-Object{"TCP $($_.LocalAddress):$($_.LocalPort) PID=$($_.OwningProcess)"}
        Get-NetUDPEndpoint -ErrorAction SilentlyContinue|Where-Object{$udpPorts -contains $_.LocalPort}|ForEach-Object{"UDP $($_.LocalAddress):$($_.LocalPort) PID=$($_.OwningProcess)"}
    )
    @("Captured=$(Get-Date -Format o)","Session=$root","ServerPID=$serverPid")+$listeners|Set-Content -LiteralPath (Join-Path $persistentDiagnostics 'status.txt') -Encoding UTF8
}
...
try {
    ...
    Ensure-MngDnsClient -LogPath $friendPreflight
    Invoke-MngDnsFlush -LogPath $friendPreflight -Reason 'before FIFA launch'
    Assert-MngLocalSenorClutch -LogPath $friendPreflight -HostsPath $hosts
    $loginHost='localhost'
    try {
        $resolved=@([Net.Dns]::GetHostAddresses('senorclutch.game')|ForEach-Object{$_.IPAddressToString})
        if($resolved -contains '127.0.0.1'){$loginHost='senorclutch.game'}
    } catch {
        Add-Content -LiteralPath $friendPreflight -Value "MNG V57 resolver fallback to localhost: $($_.Exception.Message)"
    }
    $cfg=@"
[Login]
Host=$loginHost
...
"@
    ...
    Add-Content -LiteralPath $friendPreflight -Value "MNG V57 adaptive TLS host: $loginHost"
    Write-Host "MNG V57: hote de connexion TLS -> $loginHost" -ForegroundColor Green
    ...
    $serverStdout=Join-Path $persistentDiagnostics 'local-server-stdout.log'
    $serverStderr=Join-Path $persistentDiagnostics 'local-server-stderr.log'
    $server=Start-Process -FilePath $node -WindowStyle Hidden -PassThru -ArgumentList @("`"$(Join-Path $root 'local-server.js')`"") -WorkingDirectory $root -RedirectStandardOutput $serverStdout -RedirectStandardError $serverStderr
    $ready=$false
    ...
    if($env:MNG_FUT_SERVER_ONLY -eq '1'){
        Write-Host 'MNG FUT SERVER READY. Start FIFA manually from Frosty Mod Manager.' -ForegroundColor Green
        Write-Host "Diagnostics permanents: $persistentDiagnostics" -ForegroundColor Cyan
        $nextDiagnosticCapture=Get-Date
        while(-not $server.HasExited){
            Start-Sleep -Seconds 1
            $server.Refresh()
            if((Get-Date) -ge $nextDiagnosticCapture){Save-MngPersistentDiagnostics -ServerProcess $server;$nextDiagnosticCapture=(Get-Date).AddSeconds(5)}
        }
        Save-MngPersistentDiagnostics -ServerProcess $server
        throw 'The MNG FUT local server stopped unexpectedly.'
    }
    ...
    $sessionProfiles=Join-Path $sessionDataRoot 'profiles'
    if(Test-Path -LiteralPath $sessionProfiles -PathType Container){
        Copy-Item -LiteralPath $sessionProfiles -Destination $persistentDataRoot -Recurse -Force
    }
    foreach($name in @('mng-cloud-session.json','mng-cloud-club.json')){
        $sessionFile=Join-Path $sessionDataRoot $name
        if(Test-Path -LiteralPath $sessionFile -PathType Leaf){
            Copy-Item -LiteralPath $sessionFile -Destination (Join-Path $persistentStateRoot $name) -Force
        }
    }
