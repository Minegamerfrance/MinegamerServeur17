...
# MNG FUT Frosty + senorclutch hosts/DNS self-heal V53 - 2026-09-11
...
try {
    $launchMutex=New-Object Threading.Mutex($false,'Local\MNGFUT-FIFA17-LAUNCH-V53')
    try {
...
# MNG V50 - repair broken Frosty links after a first install or after the
# selected FIFA17.exe has changed. Only broken links are touched, and only when
# the matching source exists in the CURRENT FIFA 17 folder.
function Repair-MngFrostyLinks {
    param([string]$ModRoot,[string]$CurrentGameRoot,[string]$LogPath)

    if(-not(Test-Path -LiteralPath $ModRoot -PathType Container)){ return 0 }
    ...
    $items=@()
    try {
        $items=@(Get-ChildItem -LiteralPath $ModRoot -Recurse -Force -ErrorAction SilentlyContinue |
            Where-Object { ($_.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 })
    } catch {}

    foreach($item in $items){
        $checked++
        if(-not $item.FullName.StartsWith($prefix,[StringComparison]::OrdinalIgnoreCase)){ continue }

        $relative=$item.FullName.Substring($prefix.Length)
        if([string]::IsNullOrWhiteSpace($relative)){ continue }

        $expected=Join-Path $CurrentGameRoot $relative
        if(-not(Test-Path -LiteralPath $expected)){ continue }

        $targetRaw=$null
        try {
            $targets=@($item.Target)
            if($targets.Count -gt 0){ $targetRaw=[string]$targets[0] }
        } catch {}

        $currentTarget=$null
        if(-not [string]::IsNullOrWhiteSpace($targetRaw)){
            if([IO.Path]::IsPathRooted($targetRaw)){
                $currentTarget=$targetRaw
            }else{
                $currentTarget=Join-Path (Split-Path -Parent $item.FullName) $targetRaw
            }
        }

        $broken=([string]::IsNullOrWhiteSpace([string]$currentTarget) -or -not(Test-Path -LiteralPath $currentTarget))
        if(-not $broken){ continue }

        try {
            Remove-Item -LiteralPath $item.FullName -Force -ErrorAction Stop
            New-Item -ItemType SymbolicLink -Path $item.FullName -Target $expected -Force -ErrorAction Stop | Out-Null
            $fixed++
            Add-Content -LiteralPath $LogPath -Value "FrostyLinkRepair: $relative -> $expected"
        } catch {
            Add-Content -LiteralPath $LogPath -Value "FrostyLinkRepair FAILED: $relative :: $($_.Exception.Message)"
        }
    }

    Add-Content -LiteralPath $LogPath -Value "FrostyLinkRepair summary: checked=$checked fixed=$fixed"
    return $fixed
}

if(Test-Path -LiteralPath $modDataRoot -PathType Container){
    [void](Repair-MngFrostyLinks -ModRoot $modDataRoot -CurrentGameRoot $gameDir -LogPath $friendPreflight)
}
...
'@
    ...
    do {
        ...
        if($useModData -and $launchAttempt -eq 1){
            # Frosty supports GAME_DATA_DIR as an alternative to -dataPath.
            # This avoids argument forwarding/quoting problems.
            $launchMode='GAME_DATA_DIR'
            Write-Host "MNG FUT: Frosty via GAME_DATA_DIR -> $modDataRoot" -ForegroundColor Green
            ...
            # Second Frosty mode: a relative datapath from FIFA17.exe.
            $launchMode='DATAPATH_RELATIVE'
            Write-Host 'MNG FUT: seconde tentative Frosty via -dataPath "ModData\Editor"...' -ForegroundColor Yellow
            $gameArgs=@('-dataPath','"ModData\Editor"')
            $game=Start-Process -FilePath $GameExe -WorkingDirectory $gameDir -ArgumentList $gameArgs -PassThru
        }else{
            if($useModData){
                Write-Host 'MNG FUT: les deux modes Frosty ont echoue. Test de secours sans ModData...' -ForegroundColor Yellow
            }else{
        ...
        Add-Content -LiteralPath $friendPreflight -Value "Mode lancement: tentative=$launchAttempt mode=$launchMode frostymoddata=$attemptUseModData"
        ...
        if($useModData){
            # Try GAME_DATA_DIR, then relative -dataPath, then vanilla.
            $retryEarlyNativeExit=($nativeEarlyExit -and $attemptUseModData -and $elapsedSeconds -lt 600 -and $launchAttempt -lt 3)
        }else{
        ...
        if($retryEarlyNativeExit){
            ...
            if($useModData -and $launchAttempt -eq 1){
                Write-Host 'Frosty GAME_DATA_DIR a echoue. Essai automatique avec -dataPath relatif...' -ForegroundColor Yellow
            }elseif($useModData -and $launchAttempt -eq 2){
                Write-Host 'Frosty -dataPath a echoue. Essai automatique sans ModData...' -ForegroundColor Yellow
            }else{
