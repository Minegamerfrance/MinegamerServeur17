...
function tournamentUserListDocument() {
  ...
  const ids=Object.entries(progress)
    .filter(([id,entry])=>validIds.has(Number(id))&&(
      tournamentProgressIsResumable(entry) ||
      (String(entry?.cloudRunToken||'') && entry?.completed!==true)
    ))
    .map(([id])=>Number(id))
    .sort((a,b)=>a-b);
...
function tournamentUserDocument(tournamentId) {
  ...
  const cloudRunActive=Boolean(String(entry?.cloudRunToken||''))&&entry?.completed!==true;
  if(!tournamentProgressIsResumable(entry)&&!cloudRunActive)return {tournamentId:id,TOURNAMENT_ID:id,round:1,ROUND:1,active:false};
  return {
    tournamentId:id,
    TOURNAMENT_ID:id,
...
async function handle(req,res,urlPath,requestBody) {
  ...
  if(!state.__diagHotfixSeen){
    ...
    logger('[diag] V113 native MatchReady routing active - PUT /match items[11] no longer re-enters CreateMatch');
  }
