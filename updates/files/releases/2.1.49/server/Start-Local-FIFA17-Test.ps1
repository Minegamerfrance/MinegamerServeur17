...
function Assert-MngLocalSenorClutch {
    ...
    # Some Windows installations refresh the hosts resolver asynchronously after
    # ipconfig /flushdns. Retry before considering it a real network failure.
    ...
    # The hosts file is the authoritative route used by FIFA. Do not block a
    # launch solely because a delayed or disabled Windows DNS client cannot
    # expose that mapping through the .NET resolver yet.
    if($lastResolverError -and $lastResolverError -notmatch '^Resolution invalide'){
        Add-Content -LiteralPath $LogPath -Value "senorclutch resolver indisponible, hosts local confirme: lancement continue. Detail: $lastResolverError"
        return
    }
    throw "Resolution senorclutch.game invalide : $lastResolverError Attendu uniquement 127.0.0.1."
}
