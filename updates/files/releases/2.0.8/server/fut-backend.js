...
async function discardItems(itemIds) {
  ...
  const discarded=owned.map(item=>({
    ...
    pile:0,
    itemState:'discarded',
    discardValue:valuesById.has(Number(item.id))?valuesById.get(Number(item.id)):(Number(item.discardValue)||0)
  }));
  ...
  const discardValue=Number.isFinite(Number(authorization.discardValue))?
    Number(authorization.discardValue):localDiscardTotal;
...
async function handle(req,res,urlPath,requestBody) {
  ...
  if(!state.__diagHotfixSeen){
    ...
    logger('[diag] V100 solo-cup/season secure-match state fix active - activeMatch survives authorization and reload');
  }
