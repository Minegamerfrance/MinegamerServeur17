...
function Assert-MngLocalSenorClutch {
    ...
    # The hosts file is authoritative for FIFA. Some Windows installations keep
    # a negative .NET DNS cache even after ipconfig /flushdns; do not reject a
    # valid hosts mapping only because GetHostAddresses is temporarily stale.
    ...
    do {
        try {
            ...
            if($resolved -contains '127.0.0.1' -and $bad.Count -eq 0){ return }
            $lastResolverError="Resolution invalide : $($resolved -join ', ')."
        } catch {
            $lastResolverError=$_.Exception.Message
            Add-Content -LiteralPath $LogPath -Value "senorclutch resolver en attente: $lastResolverError"
        }
        Start-Sleep -Milliseconds 500
    } while((Get-Date) -lt $deadline)

    Add-Content -LiteralPath $LogPath -Value "DNS resolver warning ignored because hosts mapping is valid: $lastResolverError"
    Write-Host 'Avertissement DNS Windows ignore : la redirection locale est correctement inscrite dans hosts.' -ForegroundColor Yellow
    return
}
