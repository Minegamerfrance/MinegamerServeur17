...
function dreamSquadPackList() {
  ...
  for(let packIndex=0;packIndex<4;packIndex++){
    ...
    packs.push({
      id:packIndex,image:packIndex,formation:'f442',manager:0,squad,
      teamId:cards.map(card=>Number(card.teamId)||0),
      Rating:cards.map(card=>Number(card.rating)||0),
      Rare:cards.map(card=>Number(card.rareFlag)||1),
      playStyle:cards.map(()=>0),
      Attribute1:attributes(0),Attribute2:attributes(1),Attribute3:attributes(2),
      Attribute4:attributes(3),Attribute5:attributes(4),Attribute6:attributes(5),
