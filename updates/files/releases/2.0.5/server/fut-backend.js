...
async function cloudListOwnedItem(body={}) {
  ...
  if(!await forceMngCloudClubSync('before-market-list')){
    return {status:503,error:'CLUB_SYNC_FAILED',reason:'CLUB_SYNC_FAILED'};
  }
  ...
  if(!payload.ok){
    logger(`[mng-market] list rejected item=${itemId} status=${payload.status} error=${payload.error||''}`);
    return {status:payload.status,error:payload.error,reason:payload.error};
  }
...
async function handle(req,res,urlPath,requestBody) {
  ...
  if(!state.__diagHotfixSeen){
    ...
    logger('[diag] V98 market/quicksell compatibility active - canonical auction list + FIFA items[] quicksell response');
  }
