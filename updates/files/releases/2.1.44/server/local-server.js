// MNG FUT 2.0.7 - squad create/rename + Util user-text filter - 2026-09-11
'use strict';
...
const onlinePacks = require('./mng-online-packs');
...
const persistentLogDir=process.env.LOCALAPPDATA
  ?path.join(process.env.LOCALAPPDATA,'MNGFUTLauncher','logs')
  :logDir;
fs.mkdirSync(persistentLogDir,{recursive:true});
const persistentLogFile=path.join(persistentLogDir,'last-local-server.log');
try{fs.writeFileSync(persistentLogFile,'','utf8');}catch(_){}
function log(message) {
  const line = `${new Date().toISOString()} ${message}`;
  fs.appendFileSync(logFile, line + '\n');
  if(persistentLogFile!==logFile){
    try{fs.appendFileSync(persistentLogFile,line+'\n');}catch(_){}
  }
  process.stdout.write(line + '\n');
}
// MNG ONLINE DYNAMIC IMAGES GITHUB V1
onlineImages.init({root,log});
onlinePacks.init({root,log});
log(`[online-pack97] status=${JSON.stringify(onlinePacks.status())}`);
...
function readUtilUserTextList(payload){
  // FIFA 17 Util/0x14 sends:
  // TLST = list<struct> [{ DIRT=2 (unchecked), UTXT="user text" }]
  // This RPC is the profanity/name-validation gate used before squad create
  // and squad rename. Without a reply the front end fails locally and never
  // sends the FUT HTTP mutation.
  const marker=tdfTag('TLST',4);
  let offset=payload.indexOf(marker);
  if(offset<0)return [];
  offset+=marker.length;

  // list element type must be struct/group (3)
  if(offset>=payload.length || payload[offset++]!==3)return [];
  ...
  for(let i=0;i<countValue.value;i++){
    let text='';
    let guard=0;
    while(offset<payload.length && guard++<64){
      if(payload[offset]===0){offset++;break;}
      if(offset+4>payload.length)break;

      const tag=payload.subarray(offset,offset+3);
      const type=payload[offset+3];
      offset+=4;

      // UTXT string
      if(tag.equals(tdfTag('UTXT',1).subarray(0,3)) && type===1){
        const lengthValue=readTdfVarIntAt(payload,offset);
        if(!lengthValue || lengthValue.value<1 || offset>payload.length)break;
        offset=lengthValue.offset;
        const byteLength=lengthValue.value;
        if(offset+byteLength>payload.length)break;
        text=payload.subarray(offset,offset+Math.max(0,byteLength-1)).toString('utf8');
        offset+=byteLength;
        continue;
      }

      // Known DIRT integer.
      if(tag.equals(tdfTag('DIRT',0).subarray(0,3)) && type===0){
        const value=readTdfVarIntAt(payload,offset);
        if(!value)break;
        offset=value.offset;
        continue;
      }

      // Captured FIFA 17 squad-name requests currently contain only DIRT+UTXT.
      // Stop on an unknown field rather than mis-parsing a future schema.
      break;
    }
    out.push(text);
  }
...
function buildUtilUserTextFilterPayload(texts){
  const groups=(Array.isArray(texts)?texts:[]).map(value=>Buffer.concat([
    // DIRT=0 => clean/accepted. FIFA sends DIRT=2 before server filtering.
    tdfInt('DIRT',0),
    tdfString('UTXT',String(value||'').slice(0,64)),
    Buffer.from([0])
  ]));
  return tdfGroupList('TLST',groups);
}
...
let matchmakingSessionSequence=1000;
    ...
    if (name === 'blaze') {
      ...
      socket.on('data', data => {
        ...
        while (buffered.length >= 16) {
          ...
          } else if (messageType === 0 && component === 0x0009 && command === 0x0014) {
            // Util user-text/profanity filter.
            // Diagnostic 2026-09-11 proved this is the missing transaction:
            // squad creation sent e.g. UTXT="fefdfsdf" and rename sent
            // UTXT="Local XI1111", then the old server logged "Unimplemented
            // frame captured" and FIFA showed an immediate create/rename error.
            const texts=readUtilUserTextList(requestPayload);
            const payload=buildUtilUserTextFilterPayload(texts);
            const response=buildFire2Reply(component,command,messageNumber,userIndex,payload);
            socket.write(response);
            log(`[squad-name-blaze] TX Util/0x14 clean text reply msg=${messageNumber} texts=${JSON.stringify(texts)} payload=${payload.length}`);
            log(`[blaze] TX hex=${response.toString('hex').toUpperCase()}`);
          ...
          } else if (messageType === 0 && component === 0x0004 && command === 0x000d) {
            // GameManager/StartMatchmaking. FIFA 17 FUT Online Seasons sends
            // this after squad validation. A valid MSID keeps the search
            // alive; the previous server never replied and the frontend
            // immediately displayed "No opponent found".
            const matchmakingSessionId=++matchmakingSessionSequence;
            const matchmakingPayload=tdfInt('MSID',matchmakingSessionId);
            const response=buildFire2Reply(component,command,messageNumber,userIndex,matchmakingPayload);
            socket.write(response);
            const requestText=requestPayload.toString('latin1').replace(/[^\x20-\x7e]+/g,' ');
            log(`[mng-1v1] StartMatchmaking accepted MSID=${matchmakingSessionId} mode=${requestText.includes('fut_online_h2h')?'fut_online_h2h':'unknown'}`);
            log(`[blaze] TX GameManager/StartMatchmaking Reply msg=${messageNumber} payload=${matchmakingPayload.length}`);
            log(`[blaze] TX hex=${response.toString('hex').toUpperCase()}`);
          } else if (messageType === 0 && component === 0x0004 && command === 0x000e) {
            const response=buildFire2Reply(component,command,messageNumber,userIndex,Buffer.alloc(0));
            socket.write(response);
            log(`[mng-1v1] CancelMatchmaking acknowledged msg=${messageNumber}`);
...
function serviceHttpHandler(name,req,res){
  ...
  req.on('end',async ()=>{
    ...
    // MNG-IMMEDIATE-WALLET-ROUTES-2.1.27
    // FIFA renders the top-bar balance from /user, /userMassInfo, /hub and
    // /user/credits. Pull Cloud before those responses instead of waiting for
    // Store/Transfer Market to trigger a later wallet reconciliation.
    if(isFut&&[
      '/ut/game/fifa17/user',
      '/ut/game/fifa17/user/list',
      '/ut/game/fifa17/user/list/',
      '/ut/game/fifa17/usermassinfo',
      '/ut/game/fifa17/user/credits',
      '/ut/game/fifa17/credits',
      '/credits',
      '/ut/game/fifa17/hub',
      '/ut/game/fifa17/user/hub'
    ].includes(urlPath)){
      await futBackend.waitForMngFullCloudHydration();
      await futBackend.refreshMngCloudWalletFromServer(`fut-entry:${urlPath}`,750);
    }
    ...
    if(name==='pas'){
        ...
        if(setImageId && fs.existsSync(customSetImage)){
          ...
          const hasDefault=fs.existsSync(tilePath);
          const body=hasDefault
            ?fs.readFileSync(tilePath)
            :Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=','base64');
          res.writeHead(200,{'content-type':'image/png','content-length':body.length,'cache-control':'no-store','connection':'close','x-mng-sbc-image-source':hasDefault?'DEFAULT':'EMBEDDED_FALLBACK'});
          res.end(body);
          log(`[sbc-set-github] V24 SERVED id=${setImageId||0} source=${hasDefault?'DEFAULT':'EMBEDDED_FALLBACK'} bytes=${body.length}`);
        }
        ...
        res.end(body);
        log(`[${name}] Served ${hasDefault?'default':'embedded fallback'} SBC challenge artwork ${urlPath}`);
    } else if(isFut && /^\/fut\/mng\/packs\/97\/(?:config\.json|background\.png|live\.png|player_\d\d\.png)$/.test(urlPath)) {
        // MNG ONLINE PACK97 ROUTE V0.1
        if(await onlinePacks.tryServe({urlPath,res,serverName:name}))return;
        res.writeHead(404,{'content-type':'application/octet-stream','content-length':'0','cache-control':'no-store','connection':'close'});
        res.end();
        log(`[online-pack97] 404 ${urlPath}`);
        ...
        if(await onlineImages.tryServePlayerHead({requestedId,extension,res,serverName:name})){
          return;
        }
    ...
    } else if(isFut && /^\/fut\/loc\/PC\/leaderboards\.[A-Za-z_]+\.xml$/i.test(urlPath)) {
        const body='<?xml version="1.0" encoding="UTF-8"?><leaderboards><leaderboard id="1" group="seasons" name="Saisons en ligne"/></leaderboards>';
        res.writeHead(200,{'content-type':'application/xml; charset=utf-8','content-length':Buffer.byteLength(body),'cache-control':'no-store'});
        res.end(body);
        log(`[${name}] FUT localized online-season leaderboard catalogue`);
