...
const DEFAULT_CONFIG={
  ...
  sbcSetImagesEnabled:true,
  sbcSetImagesPath:'sbc-set-images',
  sbcSetImageIds:[1170000,1170001],
  sbcSetImagesRefreshMinutes:5,
  refreshMinutes:5,
  timeoutMs:10000
};
...
function loadConfig(){
  let raw={};
  try{
    raw={...DEFAULT_CONFIG,...parseJson(configPath())};
  }catch(error){
    raw={...DEFAULT_CONFIG};
    log(`CONFIG_MISSING -> BUILTIN_DEFAULTS ${error.message}`);
    try{
      fs.mkdirSync(path.dirname(configPath()),{recursive:true});
      fs.writeFileSync(configPath(),JSON.stringify(raw,null,2)+'\n','utf8');
    }catch(writeError){log(`CONFIG_SELF_HEAL_WARNING ${writeError.message}`);}
  }
  const ids=Array.isArray(raw.sbcSetImageIds)?raw.sbcSetImageIds:DEFAULT_CONFIG.sbcSetImageIds;
  config={
    enabled:raw.enabled!==false && raw.sbcSetImagesEnabled!==false,
    rawBaseUrl:String(raw.rawBaseUrl||DEFAULT_CONFIG.rawBaseUrl).replace(/\/+$/,''),
    remoteDir:String(raw.sbcSetImagesPath||DEFAULT_CONFIG.sbcSetImagesPath).replace(/^\/+|\/+$/g,''),
    ids:[...new Set(ids.map(Number).filter(Number.isFinite).filter(id=>id>0))],
    refreshMinutes:Math.max(1,Number(raw.sbcSetImagesRefreshMinutes||raw.refreshMinutes)||DEFAULT_CONFIG.refreshMinutes),
    timeoutMs:Math.max(2000,Number(raw.timeoutMs)||DEFAULT_CONFIG.timeoutMs)
  };
  return config;
}
