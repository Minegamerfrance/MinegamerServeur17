...
async function startSecureMatch(body={}) {
  ...
  if(tournamentMode){
    ...
    // presses Play again.
    if(progress.completed===true || progress.won===true || !String(progress.cloudRunToken||'')){
    ...
    if(!authorization.ok){
      // Keep a 409 session alive so FIFA's own retry reuses the same matchId
      // rather than allocating 1700000003, 4, 5, 6... as the old backend did.
...
async function handle(req,res,urlPath,requestBody) {
  ...
  if(!state.__diagHotfixSeen){
    ...
    logger('[diag] V115 Solo Draft launch fix active - unique match IDs + squadId 900001 + Draft MatchReady');
  }
