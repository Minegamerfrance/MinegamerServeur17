...
// RELEASE 2.1.43 - ICONES PACK 318: 5500 FP + one local rarity-12 Icon
'use strict';
...
// Server sessions are disposable and the launcher deliberately deletes them
// before each start. Store the complete FUT club outside that temporary tree
// so players, squads, packs and consumables survive logout and launcher swaps.
const MNG_PERSISTENT_DATA_ROOT = process.env.LOCALAPPDATA
  ? path.join(process.env.LOCALAPPDATA, 'MNGFUTLauncher', 'server-data')
  : path.join(ROOT, 'data');
const STATE_PATH = process.env.FIFA17_FUT_STATE_PATH || path.join(MNG_PERSISTENT_DATA_ROOT, 'profiles', activeCloudProfileKey(), 'local-fut-state.json');
...
// MNG-IMMEDIATE-WALLET-2.1.27
// Read the authoritative wallet directly from the authenticated Cloud profile.
// This is a PULL only: /api/me never writes the wallet, so entering FUT cannot
// accidentally overwrite newer Coins/FIFA Points with a stale local cache.
let MNG_CLOUD_WALLET_REFRESH_PROMISE = null;
let MNG_CLOUD_WALLET_REFRESHED_AT = 0;
async function refreshMngCloudWalletFromServer(reason='fut-entry',maxAgeMs=1000) {
  if(!MNG_CLOUD_PROFILE||!state||!MNG_CLOUD_PROFILE.apiBaseUrl||!MNG_CLOUD_PROFILE.token)return false;
  const maxAge=Math.max(0,Math.floor(Number(maxAgeMs)||0));
  if(maxAge>0&&MNG_CLOUD_WALLET_REFRESHED_AT>0&&(Date.now()-MNG_CLOUD_WALLET_REFRESHED_AT)<maxAge)return true;
  if(MNG_CLOUD_WALLET_REFRESH_PROMISE)return await MNG_CLOUD_WALLET_REFRESH_PROMISE;
  MNG_CLOUD_WALLET_REFRESH_PROMISE=(async()=>{
    const controller=typeof AbortController==='function'?new AbortController():null;
    const timeout=controller?setTimeout(()=>controller.abort(),8000):null;
    try{
      if(typeof fetch!=='function')throw new Error('Node fetch unavailable');
      const response=await fetch(`${MNG_CLOUD_PROFILE.apiBaseUrl}/api/me`,{
        method:'GET',
        headers:{'accept':'application/json','authorization':`Bearer ${MNG_CLOUD_PROFILE.token}`},
        signal:controller?controller.signal:undefined
      });
      ...
      applyCloudWalletProfile(payload.profile);
      writeStateDirectly(state);
      MNG_CLOUD_WALLET_REFRESHED_AT=Date.now();
      logger(`[mng-cloud] V2.1.27 wallet pulled reason=${reason} coins=${state.coins} points=${state.points} revision=${MNG_CLOUD_PROFILE.walletRevision}`);
      return true;
    }catch(error){
      logger(`[mng-cloud] V2.1.27 wallet pull failed reason=${reason}: ${error.message}`);
      return false;
    }finally{
      if(timeout)clearTimeout(timeout);
    }
  })();
  try{return await MNG_CLOUD_WALLET_REFRESH_PROMISE;}
  finally{MNG_CLOUD_WALLET_REFRESH_PROMISE=null;}
}
...
let MNG_CLOUD_CLUB_SAFE_ITEM_COUNT = 0;
let MNG_CLOUD_LAST_CLUB_KEY = '';
// V99: keep the exact rejection reason so market/quicksell can distinguish
// an unauthorized local-inventory delta from a real network/cloud outage.
let MNG_CLOUD_CLUB_LAST_ERROR = '';
...
function applyMngCloudClubCache() {
  ...
  MNG_CLOUD_CLUB_SAFE_ITEM_COUNT=cloudClub.items.length;
...
async function syncMngCloudClub(reason='saveState') {
  ...
  const currentItemCount=Array.isArray(club.items)?club.items.length:0;
  const destructiveThreshold=MNG_CLOUD_CLUB_SAFE_ITEM_COUNT>=40
    ? Math.floor(MNG_CLOUD_CLUB_SAFE_ITEM_COUNT*0.6)
    : 0;
  if(destructiveThreshold>0&&currentItemCount<destructiveThreshold){
    MNG_CLOUD_CLUB_LAST_ERROR='DESTRUCTIVE_CLUB_SYNC_BLOCKED';
    logger(`[mng-cloud] destructive club sync blocked reason=${reason} current=${currentItemCount} safe=${MNG_CLOUD_CLUB_SAFE_ITEM_COUNT}`);
    return;
  }
  ...
  try{
    ...
    if(response.status===409&&payload&&payload.error==='CLUB_CONFLICT'){
      ...
      MNG_CLOUD_CLUB_SYNC_PENDING=false;
      throw new Error('CLUB_CONFLICT_NEWER_CLOUD_PRESERVED');
    }
    if(!response.ok||!payload||payload.ok!==true){
    ...
    MNG_CLOUD_CLUB_SAFE_ITEM_COUNT=Math.max(MNG_CLOUD_CLUB_SAFE_ITEM_COUNT,currentItemCount);
    MNG_CLOUD_LAST_CLUB_KEY=key;
    MNG_CLOUD_CLUB_LAST_ERROR='';
    try{
      fs.writeFileSync(MNG_CLOUD_CLUB_PATH,JSON.stringify({version:1,personaId:Number(PERSONA_ID)||0,revision:MNG_CLOUD_CLUB_REVISION,club,savedAt:new Date().toISOString()},null,2),'utf8');
    }catch(cacheError){logger(`[mng-cloud] club cache write failed: ${cacheError.message}`);}
    ...
    MNG_CLOUD_CLUB_LAST_ERROR=String(error?.message||error||'CLUB_SYNC_FAILED');
    logger(`[mng-cloud] V2.2 club sync failed reason=${reason}: ${MNG_CLOUD_CLUB_LAST_ERROR}`);
  }finally{
...
// ---------------------------------------------------------------------------
// MNG FUT 2.1.0 - FULL CLOUD SAVE
// ---------------------------------------------------------------------------
// The legacy wallet and /api/club endpoints remain enabled for transactional
// compatibility. This additional save stores the COMPLETE local FUT state:
// inventory, squads, SBC completion, reward packs, market pile, Draft,
// offline seasons/tournaments and club identity. It is stored per MNG account.
//
// The full-save Worker is intentionally a separate Cloudflare Worker so it can
// be deployed without replacing the existing login/wallet API.
const MNG_FULL_SAVE_API_BASE_URL =
  String(process.env.MNG_FULL_SAVE_API_BASE_URL || 'https://mng-fut-full-save.minegamerfrance.workers.dev').replace(/\/+$/,'');

let MNG_FULL_SAVE_SUPPORTED = false;
let MNG_FULL_SAVE_READY = false;
let MNG_FULL_SAVE_REVISION = 0;
let MNG_FULL_SAVE_LAST_KEY = '';
let MNG_FULL_SAVE_TIMER = null;
let MNG_FULL_SAVE_IN_FLIGHT = false;
let MNG_FULL_SAVE_PENDING = false;
let MNG_FULL_SAVE_APPLYING = false;
let MNG_FULL_SAVE_CONFLICT = false;
let MNG_FULL_SAVE_HYDRATION_PROMISE = null;
let MNG_FULL_SAVE_SAFE_ITEM_COUNT = 0;

function fullCloudStateSnapshot() {
  const cleanState=JSON.parse(JSON.stringify(state||{}));
  delete cleanState.__diagHotfixSeen;
  return {
    schemaVersion:1,
    game:'fifa17',
    personaId:Number(PERSONA_ID)||0,
    userId:Number(MNG_CLOUD_PROFILE?.userId)||0,
    clubName:String(CLUB_NAME||cleanState.clubName||'').slice(0,24),
    clubAbbr:String(CLUB_ABBR||cleanState.clubAbbr||'').slice(0,3),
    wallet:{
      coins:Math.max(0,Math.floor(Number(cleanState.coins)||0)),
      fifaPoints:Math.max(0,Math.floor(Number(cleanState.points)||0)),
      walletRevision:Math.max(0,Math.floor(Number(MNG_CLOUD_PROFILE?.walletRevision)||0))
    },
    state:cleanState,
...
function fullCloudSaveKey(snapshot=fullCloudStateSnapshot()) {
  try{
    const stable=JSON.stringify(snapshot?.state||{});
...
function validateFullCloudSnapshot(save) {
  if(!save||typeof save!=='object')throw new Error('FULL_SAVE_INVALID');
  if(Number(save.schemaVersion)!==1)throw new Error('FULL_SAVE_SCHEMA_UNSUPPORTED');
  if(String(save.game||'fifa17')!=='fifa17')throw new Error('FULL_SAVE_WRONG_GAME');
  if(Number(save.personaId)>0&&Number(save.personaId)!==Number(PERSONA_ID))throw new Error('FULL_SAVE_WRONG_PERSONA');

  const incoming=save.state;
  if(!incoming||typeof incoming!=='object'||Number(incoming.version)!==1||!Array.isArray(incoming.items)){
    throw new Error('FULL_SAVE_STATE_INVALID');
  }

  const json=JSON.stringify(incoming);
  if(Buffer.byteLength(json,'utf8')>12*1024*1024)throw new Error('FULL_SAVE_TOO_LARGE');
  return JSON.parse(json);
}

function writeStateDirectly(nextState) {
...
function backupLocalStateBeforeFullRestore() {
  try{
    if(!fs.existsSync(STATE_PATH))return '';
    const dir=path.join(path.dirname(STATE_PATH),'cloud-restore-backups');
    fs.mkdirSync(dir,{recursive:true});
    const stamp=new Date().toISOString().replace(/[:.]/g,'-');
    const target=path.join(dir,`before-full-cloud-restore-${stamp}.json`);
    fs.copyFileSync(STATE_PATH,target);
    return target;
  }catch(error){
    logger(`[full-cloud] local backup warning: ${error.message}`);
    return '';
  }
}

async function mngFullSaveRequest(method,body=undefined,timeoutMs=15000) {
  if(!MNG_CLOUD_PROFILE?.token)return {ok:false,status:401,error:'MNG_CLOUD_LOGIN_REQUIRED'};
  if(typeof fetch!=='function')return {ok:false,status:503,error:'NODE_FETCH_UNAVAILABLE'};

  const controller=typeof AbortController==='function'?new AbortController():null;
  const timeout=controller?setTimeout(()=>controller.abort(),timeoutMs):null;
  try{
    const response=await fetch(`${MNG_FULL_SAVE_API_BASE_URL}/api/save/full`,{
      method,
      headers:{
    ...
    const payload=await response.json().catch(()=>({}));
    return {ok:response.ok&&payload?.ok===true,status:response.status,payload,error:String(payload?.error||'')};
  }catch(error){
    return {ok:false,status:503,payload:null,error:String(error?.message||'FULL_SAVE_UNAVAILABLE')};
  }finally{
    if(timeout)clearTimeout(timeout);
  }
}

async function hydrateMngFullCloudSave() {
  if(!MNG_CLOUD_PROFILE){
    MNG_FULL_SAVE_READY=true;
    return false;
  }

  logger(`[full-cloud] V2.1 hydrate start endpoint=${MNG_FULL_SAVE_API_BASE_URL}`);
  const result=await mngFullSaveRequest('GET',undefined,12000);

  // The dedicated Worker exists but this account has no save yet.
  if(result.status===404&&result.error==='SAVE_NOT_FOUND'){
    MNG_FULL_SAVE_SUPPORTED=true;
    MNG_FULL_SAVE_READY=true;
    MNG_FULL_SAVE_REVISION=0;
    MNG_FULL_SAVE_LAST_KEY='';
    logger('[full-cloud] V2.1 no remote save; current local club will seed revision 1');
    await syncMngFullCloudSave('initial-local-seed');
    return false;
  }

  if(!result.ok){
    MNG_FULL_SAVE_READY=true;
    MNG_FULL_SAVE_SUPPORTED=false;
    logger(`[full-cloud] V2.1 hydrate unavailable status=${result.status} error=${result.error||'UNKNOWN'}; legacy cloud sync remains active`);
    return false;
  }

  try{
    const remote=result.payload?.save;
    const incoming=validateFullCloudSnapshot(remote);
    const backup=backupLocalStateBeforeFullRestore();

    MNG_FULL_SAVE_APPLYING=true;
    writeStateDirectly(incoming);
    loadState();

    if(String(remote.clubName||'').trim())CLUB_NAME=String(remote.clubName).trim().slice(0,24);
    else if(String(state?.clubName||'').trim())CLUB_NAME=String(state.clubName).trim().slice(0,24);

    if(String(remote.clubAbbr||'').trim())CLUB_ABBR=String(remote.clubAbbr).trim().replace(/[^A-Za-z0-9]/g,'').toUpperCase().slice(0,3)||CLUB_ABBR;
    else if(String(state?.clubAbbr||'').trim())CLUB_ABBR=String(state.clubAbbr).trim().replace(/[^A-Za-z0-9]/g,'').toUpperCase().slice(0,3)||CLUB_ABBR;

    // Wallet remains transaction-authoritative in the existing MNG Cloud API.
    // The full snapshot stores wallet values too, but a newer wallet revision
    // must never be overwritten by a stale full save.
    state.coins=Math.max(0,Math.floor(Number(MNG_CLOUD_PROFILE.coins)||0));
    state.points=Math.max(0,Math.floor(Number(MNG_CLOUD_PROFILE.fifaPoints)||0));
    state.clubName=CLUB_NAME;
    state.clubAbbr=CLUB_ABBR;
    writeStateDirectly(state);

    MNG_FULL_SAVE_REVISION=Math.max(0,Math.floor(Number(result.payload?.revision)||0));
    MNG_FULL_SAVE_SAFE_ITEM_COUNT=Array.isArray(state.items)?state.items.length:0;
    MNG_FULL_SAVE_SUPPORTED=true;
    MNG_FULL_SAVE_READY=true;
    MNG_FULL_SAVE_CONFLICT=false;
    MNG_FULL_SAVE_LAST_KEY=fullCloudSaveKey(fullCloudStateSnapshot());

    logger(`[full-cloud] V2.1 restored revision=${MNG_FULL_SAVE_REVISION} items=${state.items.length} squads=${Array.isArray(state.squads)?state.squads.length:0} sbc=${Object.keys(state).filter(k=>/^sbc/i.test(k)&&state[k]===true).length} backup=${backup||'none'}`);
    return true;
  }catch(error){
    MNG_FULL_SAVE_READY=true;
    MNG_FULL_SAVE_SUPPORTED=false;
    logger(`[full-cloud] V2.1 restore rejected: ${error.message}; local save kept`);
    return false;
  }finally{
    MNG_FULL_SAVE_APPLYING=false;
  }
}

async function syncMngFullCloudSave(reason='saveState') {
  if(!MNG_FULL_SAVE_SUPPORTED||!MNG_FULL_SAVE_READY||MNG_FULL_SAVE_APPLYING||MNG_FULL_SAVE_CONFLICT||!MNG_CLOUD_PROFILE||!state)return false;

  if(MNG_FULL_SAVE_IN_FLIGHT){
    MNG_FULL_SAVE_PENDING=true;
    return false;
  }

  const save=fullCloudStateSnapshot();
  const key=fullCloudSaveKey(save);
  if(key&&key===MNG_FULL_SAVE_LAST_KEY&&!MNG_FULL_SAVE_PENDING)return true;

  const currentItemCount=Array.isArray(save?.state?.items)?save.state.items.length:0;
  const destructiveThreshold=MNG_FULL_SAVE_SAFE_ITEM_COUNT>=40
    ? Math.floor(MNG_FULL_SAVE_SAFE_ITEM_COUNT*0.6)
    : 0;
  if(destructiveThreshold>0&&currentItemCount<destructiveThreshold){
    logger(`[full-cloud] destructive full save blocked reason=${reason} current=${currentItemCount} safe=${MNG_FULL_SAVE_SAFE_ITEM_COUNT}`);
    return false;
  }

  MNG_FULL_SAVE_IN_FLIGHT=true;
  MNG_FULL_SAVE_PENDING=false;
  try{
    const result=await mngFullSaveRequest('PUT',{
      save,
      expectedRevision:MNG_FULL_SAVE_REVISION,
      reason:String(reason||'saveState').slice(0,120),
      source:'fifa17-local'
    },20000);

    if(result.status===409&&result.error==='SAVE_CONFLICT'){
      MNG_FULL_SAVE_CONFLICT=true;
      logger(`[full-cloud] V2.1 SAVE_CONFLICT localRevision=${MNG_FULL_SAVE_REVISION} cloudRevision=${Number(result.payload?.currentRevision)||0}; upload stopped to protect newer cloud save`);
      return false;
    }

    if(!result.ok){
      logger(`[full-cloud] V2.1 sync failed reason=${reason} status=${result.status} error=${result.error||'UNKNOWN'}`);
      return false;
    }

    MNG_FULL_SAVE_REVISION=Math.max(1,Math.floor(Number(result.payload?.revision)||MNG_FULL_SAVE_REVISION+1));
    MNG_FULL_SAVE_SAFE_ITEM_COUNT=Math.max(MNG_FULL_SAVE_SAFE_ITEM_COUNT,currentItemCount);
    MNG_FULL_SAVE_LAST_KEY=key;
    logger(`[full-cloud] V2.1 synced reason=${reason} revision=${MNG_FULL_SAVE_REVISION} bytes=${Number(result.payload?.byteSize)||0} items=${state.items.length} squads=${Array.isArray(state.squads)?state.squads.length:0}`);
    return true;
  }finally{
    MNG_FULL_SAVE_IN_FLIGHT=false;
    if(MNG_FULL_SAVE_PENDING&&!MNG_FULL_SAVE_CONFLICT){
      MNG_FULL_SAVE_PENDING=false;
      queueMngFullSaveSync('pending-change',150);
    }
  }
}

function queueMngFullSaveSync(reason='saveState',delayMs=900) {
  if(!MNG_FULL_SAVE_SUPPORTED||!MNG_FULL_SAVE_READY||MNG_FULL_SAVE_APPLYING||MNG_FULL_SAVE_CONFLICT||!MNG_CLOUD_PROFILE||!state)return;
  ...
  if(MNG_FULL_SAVE_TIMER)clearTimeout(MNG_FULL_SAVE_TIMER);
  MNG_FULL_SAVE_TIMER=setTimeout(()=>{
    MNG_FULL_SAVE_TIMER=null;
    syncMngFullCloudSave(reason);
  },Math.max(100,Number(delayMs)||900));
}

async function waitForMngFullCloudHydration() {
  if(!MNG_FULL_SAVE_HYDRATION_PROMISE)return;
  try{await MNG_FULL_SAVE_HYDRATION_PROMISE;}catch(_){}
}
...
const RIBERY_SBC_RARE_FLAG = 24; // RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry SBC rarity
const RIBERY_SBC_CARD = {assetId:156616,name:'Franck RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',rating:87,position:'LM',positionId:16,teamId:21,leagueId:19,nation:18,attributes:[85,79,85,91,28,61],quality:'gold',resourceId:RIBERY_SBC_RESOURCE_ID,definitionId:RIBERY_SBC_RESOURCE_ID,version:5,rareFlag:24,cardType:'sbc',cardTypeName:'SBC Reward'};
...
// player on the client (Effenberg -> EVANGELISTA, MatthÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¤us -> BLACKMAN), and
...
const V38_REMOVED_ICON_ASSET_IDS = new Set([
  ...
  191190, // broken MatthÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¤us slot -> Blackman
...
const V38_REMOVED_ICON_RESOURCE_IDS = new Set([
  ...
  16992775,16968406, // broken Effenberg / MatthÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¤us resources
...
const JUNINHO_ICON_ASSET_ID = 30;
const JUNINHO_ICON_RESOURCE_ID = 16777246;
const JUNINHO_ICON_CARD = {
  assetId:JUNINHO_ICON_ASSET_ID,
  name:'Juninho',
  displayName:'Juninho', commonName:'Juninho', knownAs:'Juninho', firstName:'Juninho', lastName:'', surname:'Juninho', shortName:'Juninho',
  rating:88,
  position:'CAM',
  positionId:18,
  teamId:ICON_CLUB_ID,
  ...
  leagueName:'Icons',
  nation:54,
  attributes:[77,83,90,86,68,76],
  quality:'gold',
  resourceId:JUNINHO_ICON_RESOURCE_ID,
  definitionId:JUNINHO_ICON_RESOURCE_ID,
  version:1,
  rareFlag:12,
  rareflag:12,
  cardType:'icon',
...
const RIBERY_SPECIAL_CARD = {
  assetId:156616,
  name:'Franck RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',
  displayName:'Franck RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry', commonName:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry', knownAs:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry', firstName:'Franck', lastName:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry', surname:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry', shortName:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',
  rating:90,
  position:'LM',
  positionId:16,
  teamId:21,
...
const KAKA_SPECIAL_CARD = {
  assetId:138449,
  name:'KakÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡',
  displayName:'KakÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡', commonName:'KakÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡', knownAs:'KakÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡', firstName:'Ricardo', lastName:'KakÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡', surname:'KakÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡', shortName:'KakÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡',
  rating:89,
  position:'CAM',
  positionId:18,
  teamId:47,
...
const COMMUNITY_SBC_CARDS = [
  ...
  {setId:96006,challengeId:960061,assetId:150418,resourceId:100813714,name:'Mario GÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³mez',shortName:'Gomez',rating:88,position:'ST',positionId:25,teamId:21,leagueId:19,nation:21,attributes:[83,90,73,82,43,83],skillMoves:3,weakFoot:4,requiredRating:84},
  ...
  {assetId:1625,resourceId:100664921,name:'Thierry Henry',shortName:'Henry',rating:91,position:'ST',positionId:25,teamId:1,leagueId:13,nation:18,attributes:[93,90,82,89,51,78],skillMoves:4,weakFoot:4,icon:true},
  // V3.7: exact ratings/stats requested by the user for the three Frosty/starhead Icons.
  {assetId:190049,resourceId:100853345,name:'EusÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©bio',shortName:'EusÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©bio',rating:91,position:'ST',positionId:25,teamId:ICON_CLUB_ID,leagueId:ICON_LEAGUE_ID,nation:38,attributes:[92,92,84,91,44,77],skillMoves:4,weakFoot:5,preferredFoot:'right',icon:true},
  ...
  {assetId:190048,resourceId:100853344,name:'Gerd MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼ller',shortName:'MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼ller',rating:92,position:'ST',positionId:25,teamId:ICON_CLUB_ID,leagueId:ICON_LEAGUE_ID,nation:21,attributes:[87,92,76,85,44,74],skillMoves:4,weakFoot:4,preferredFoot:'right',icon:true},
  {assetId:261593,resourceId:17038809,name:'JÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼rgen Kohler',shortName:'Kohler',rating:89,position:'CB',positionId:5,teamId:ICON_CLUB_ID,leagueId:ICON_LEAGUE_ID,nation:21,attributes:[71,51,63,62,91,90],skillMoves:2,weakFoot:3,preferredFoot:'right',icon:true},
...
const DANI_ALVES_SBC_REQUIREMENTS = [
  {type:'NATION_COUNT',scope:'SQUAD',nationId:54,min:2,value:2,name:'Joueurs brÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©siliens : minimum 2'},
  ...
  {type:'TEAM_RATING',scope:'SQUAD',min:83,value:83,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 83'},
  {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:70,value:70,name:'Collectif : minimum 70'},
  {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,exact:true,name:"Nombre de joueurs dans l'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : 11"}
];
DANI_ALVES_SBC_REQUIREMENTS[0].name='Joueurs bresiliens : minimum 2';
DANI_ALVES_SBC_REQUIREMENTS[1].name='Joueurs Serie A : minimum 3';
DANI_ALVES_SBC_REQUIREMENTS[2].name='Joueurs TOTW : minimum 1';
DANI_ALVES_SBC_REQUIREMENTS[3].name='Note equipe : minimum 83';
DANI_ALVES_SBC_REQUIREMENTS[4].name='Collectif : minimum 70';
DANI_ALVES_SBC_REQUIREMENTS[5].name="Nombre de joueurs dans l'equipe : 11";
...
// Release 2.0.2: canonical LEGENDES pack pool. These are the 18 rarity-12
// Icons that survive the active V3.8 runtime cleanup. Alex Hunter (100990001)
// stays excluded because he is an SBC-only reward.
const MNG_LEGEND_PACK_RESOURCE_IDS = Object.freeze([
  100664921, // Thierry Henry
  100853345, // Eusebio
  100911442, // Ian Wright
  100910813, // John Barnes
  16805347,  // Ronaldinho
  16778613,  // Zinedine Zidane
  16780863,  // Michael Ballack
  16778116,  // Raymond Kopa
  16778117,  // Fabien Barthez
  16794113,  // Daniel Van Buyten
  100663784, // Oliver Kahn
  100853344, // Gerd Muller
  17038809,  // Jurgen Kohler
  16884931,  // Lucio
  117575967, // Maicon
  16788357,  // Miroslav Klose
  100673831, // Xavi
  16966578   // Hulk
]);
const MNG_LEGEND_PACK_RESOURCE_SET = new Set(MNG_LEGEND_PACK_RESOURCE_IDS);
const WEEKLY_TOTW_PACK_ID = 312;
const STARTER_TOTW_PACK_ID = 315;
const STARTER_OTW_PACK_ID = 316;
const MNG_LIVE_PACK97_ID = 317;
const MNG_LIVE_PACK97_ASSET_ID = 97;
// MNG ONLINE PACK97 V0.1
...
const DRAFT_OPPONENT_TEAMS = [
  ...
  {teamId:21,teamName:'FC Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen',teamAbbr:'FCB'},
...
const PACKS = {
  ...
  318: {id:318, name:'ICONES', tier:'gold', count:1, players:1, rares:1, coins:500000, points:5500, premium:true, specialChance:1, guaranteedLegends:1, maxPurchases:0, displayGroup:'special'},
  317: {id:317, name:'MNG LIVE PACK 97', tier:'gold', count:1, players:1, rares:1, coins:0, points:0, premium:true, specialChance:1, guaranteedSpecials:1, displayGroup:'special'}
};
...
const PRESENTATION_ITEMS = [
  ...
  {id:1700000001,resourceId:6300005,assetId:14,teamid:21,teamId:21,cardassetid:35,cardsubtypeid:9,itemType:'kit',family:'kits',table:'fcc_kitcards',rating:87,rareflag:1,category:2,displayName:'Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen Home',defaultActiveState:'activeHomeKit',itemState:'activeHomeKit'},
  {id:1700000002,resourceId:6400004,assetId:15,teamid:21,teamId:21,cardassetid:35,cardsubtypeid:9,itemType:'kit',family:'kits',table:'fcc_kitcards',rating:87,rareflag:1,category:3,displayName:'Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen Away',defaultActiveState:'activeAwayKit',itemState:'activeAwayKit'},
  {id:1700000003,resourceId:6000004,assetId:21,teamid:21,teamId:21,cardassetid:39,cardsubtypeid:11,itemType:'custom',family:'badges',table:'fcc_badgecards',rating:87,rareflag:1,category:1,displayName:'Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen',defaultActiveState:'activeBadge',itemState:'activeBadge'},
...
function loadFifa17ManagerCatalogue() {
  ...
  try {
    ...
    return [
      {managerId:89,staffId:89,pictureId:89,cardDbId:1000089,assetId:1000089,resourceId:1000089,definitionId:1000089,headId:232298,headAssetId:232298,firstName:'ArsÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨ne',lastName:'Wenger',name:'ArsÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨ne Wenger',rating:86,value:86,rareflag:1,rareFlag:1,nation:18,teamid:1,teamId:1,leagueId:13,managerLeagueId:13,formationId:0,formationid:0,talkRating:0,negotiation:3,contractBoost:3,quality:'gold'},
      ...
      {managerId:414,staffId:414,pictureId:414,cardDbId:1000414,assetId:1000414,resourceId:1000414,definitionId:1000414,headId:232302,headAssetId:232302,firstName:'JÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼rgen',lastName:'Klopp',name:'JÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼rgen Klopp',rating:84,value:84,rareflag:1,rareFlag:1,nation:21,teamid:9,teamId:9,leagueId:13,managerLeagueId:13,formationId:0,formationid:0,talkRating:0,negotiation:3,contractBoost:3,quality:'gold'},
...
const CLUB_ITEM_CARDS = [
  {resourceId:6300005,assetId:14,teamid:21,cardassetid:35,cardsubtypeid:9,itemType:'kit',family:'kits',table:'fcc_kitcards',rating:87,rareflag:1,category:2,displayName:'Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen Home',defaultActiveState:'activeHomeKit'},
  {resourceId:6400004,assetId:15,teamid:21,cardassetid:35,cardsubtypeid:9,itemType:'kit',family:'kits',table:'fcc_kitcards',rating:87,rareflag:1,category:3,displayName:'Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen Away',defaultActiveState:'activeAwayKit'},
  {resourceId:6000004,assetId:21,teamid:21,cardassetid:39,cardsubtypeid:11,itemType:'custom',family:'badges',table:'fcc_badgecards',rating:87,rareflag:1,category:1,displayName:'Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen',defaultActiveState:'activeBadge'},
...
function syncOwnedHenryUpgrade() {
  ...
  for (const item of state.items) {
    ...
    const identityCorrect = Number(item.assetId)===1625 && Number(item.rareFlag ?? item.rareflag)===12 &&
      Number(item.teamId ?? item.teamid)===1 && Number(item.leagueId)===13 &&
    ...
    item.teamid=1; item.teamId=1; item.leagueId=13; item.leagueid=13; item.league=13; item.leagueName='Premier League';
...
function loadCatalog() {
  if (!catalog) {
    ...
    if(!catalog.specials.some(card=>Number(card.resourceId)===RIBERY_SBC_RESOURCE_ID))catalog.specials.push({...RIBERY_SBC_CARD,displayName:RIBERY_SBC_CARD.name,commonName:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',knownAs:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',firstName:'Franck',lastName:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',surname:'RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry'});
    ...
    catalog.base=(catalog.base||[]).filter(card=>Number(card.assetId)!==JUNINHO_ICON_ASSET_ID && Number(card.resourceId)!==JUNINHO_ICON_RESOURCE_ID);
    catalog.specials=(catalog.specials||[]).filter(card=>Number(card.assetId)!==JUNINHO_ICON_ASSET_ID && Number(card.resourceId)!==JUNINHO_ICON_RESOURCE_ID);
    catalog.specials.push({...JUNINHO_ICON_CARD});
    if(catalog.counts&&typeof catalog.counts==='object'){
      catalog.counts.base=catalog.base.length;
      catalog.counts.specials=catalog.specials.length;
      catalog.counts.total=catalog.base.length+catalog.specials.length;
    }
    logger(`[juninho-icon] registered assetId=${JUNINHO_ICON_ASSET_ID} resourceId=${JUNINHO_ICON_RESOURCE_ID} rating=88 position=CAM stats=77,83,90,86,68,76 teamId=${ICON_CLUB_ID} leagueId=${ICON_LEAGUE_ID} rareFlag=12`);
    ...
    catalog.legends=(catalog.legends||[]).filter(card=>Number(card.assetId)!==JUNINHO_ICON_ASSET_ID && Number(card.resourceId)!==JUNINHO_ICON_RESOURCE_ID);
    catalog.legends.push({...JUNINHO_ICON_CARD});
...
function repairSinglePlayerSeason(value) {
  ...
  const activeMatchValid=Boolean(
    repaired.activeMatch &&
    typeof repaired.activeMatch==='object' &&
    Number(repaired.activeMatch.id)>0 &&
    repaired.activeMatch.completed!==true
  );

  // V100: a current local/solo-cup match is intentionally persisted with
  // season.data empty. Older repair code interpreted that exact state as a
  // broken legacy resume and deleted activeMatch while /api/matches/start was
  // still being authorized, producing:
  //   TypeError: Cannot set properties of null (setting 'cloudToken')
  //
  // Only clear a legacy resume when there is no valid active match to protect.
  const hasBrokenResume=Boolean(repaired.started) && !hasResumePayload &&
    repaired.gamesPlayed>0 && !activeMatchValid;
  if(hasBrokenResume){
    const clean=freshSinglePlayerSeason(repaired.divisionId,repaired.totals);
    clean.nextMatchId=repaired.nextMatchId;
    logger(`[offline-season] V100 cleared incomplete legacy resume division=${repaired.divisionId}`);
    return clean;
  }

  if(activeMatchValid){
    repaired.started=true;
    repaired.activeMatch.id=Number(repaired.activeMatch.id);
    repaired.activeMatch.round=Math.max(1,Number(repaired.activeMatch.round)||repaired.gamesPlayed+1);
    repaired.activeMatch.teamId=Math.max(1,Number(repaired.activeMatch.teamId)||1);
    repaired.activeMatch.createdAt=Math.max(0,Number(repaired.activeMatch.createdAt)||Date.now());
    repaired.activeMatch.mode=String(repaired.activeMatch.mode||'season');
    repaired.activeMatch.tournamentId=Math.max(0,Number(repaired.activeMatch.tournamentId)||0);
    repaired.activeMatch.cloudToken=String(repaired.activeMatch.cloudToken||'');
  }
  return repaired;
}
...
function saveState() {
  ...
  queueMngFullSaveSync('saveState');
}
...
function init(logFunction) {
  ...
  loadState();
  if(normalizeOwnedGoldRareQuickSellValues()>0){
    // At this point cloud sync has not been enabled yet, so this is a safe
    // local metadata migration and cannot create an inventory-sync conflict.
    try{
      fs.mkdirSync(path.dirname(STATE_PATH),{recursive:true});
      const temp=`${STATE_PATH}.tmp`;
      fs.writeFileSync(temp,JSON.stringify(state));
      ...
      logger(`[quicksell-gold-rare] migration save failed: ${error.message}`);
    }
  }
  ...
  if(cloudProfile){
    ...
    // 2.1.27: the JSON session is only a cache. Do not mark its wallet as
    // already authoritative before the server has been queried.
    MNG_CLOUD_LAST_WALLET_KEY='';
    ...
    // Start complete account hydration immediately. The first FUT request waits
    // for this promise, so a new PC cannot render a starter club before its
    // existing cloud club has been restored.
    // Refresh the authoritative wallet BEFORE applying the full cloud save.
    // hydrateMngFullCloudSave deliberately reuses MNG_CLOUD_PROFILE for Coins,
    // therefore the order here prevents a stale cached balance at FUT login.
    MNG_FULL_SAVE_HYDRATION_PROMISE=(async()=>{
      await refreshMngCloudWalletFromServer('startup',0);
      return await hydrateMngFullCloudSave();
    })();
  }else{
    MNG_CLOUD_SYNC_ENABLED=false;
    ...
    MNG_FULL_SAVE_READY=true;
    MNG_FULL_SAVE_SUPPORTED=false;
    MNG_FULL_SAVE_HYDRATION_PROMISE=null;
...
function goldRareQuickSellValue(card) {
  if(!card || typeof card!=='object')return null;

  const itemType=String(card.itemType||'player').toLowerCase();
  if(itemType!=='player')return null;

  const quality=String(card.quality||'').toLowerCase();
  const rareFlag=Number(card.rareFlag??card.rareflag??0);
  const version=Number(card.version)||0;
  const cardType=String(card.cardType||'base').toLowerCase();

  // Only NORMAL rare gold players. Special cards (TOTW, Icon, SBC, OTW...)
  // keep their own quick-sell systems.
  if(quality!=='gold'||rareFlag!==1||version!==0||!['','base','normal'].includes(cardType))return null;

  // Mineg requested the full rare-gold range 658 -> 921.
  // Rating 75 is the floor and FIFA 17's top normal gold rating (94) is
  // the ceiling. Ratings outside that range are clamped.
  const rating=Math.max(75,Math.min(94,Number(card.rating)||75));
  const progress=(rating-75)/(94-75);
  return Math.round(658+(921-658)*progress);
}

function playerQuickSellValue(card,{untradeable=false}={}) {
  if(untradeable===true || card?.untradeable===true || card?.tradeable===false)return 0;

  const totw=totwQuickSellValue(card);
  if(totw!==null)return totw;

  const goldRare=goldRareQuickSellValue(card);
  if(goldRare!==null)return goldRare;

  const rating=Math.max(0,Number(card?.rating)||0);
  return Math.max(10,Math.floor(Math.pow(Math.max(0,rating-40),2)/20));
}

function normalizeOwnedGoldRareQuickSellValues() {
  if(!state||!Array.isArray(state.items))return 0;
  let changed=0;
  for(const item of state.items){
    const target=goldRareQuickSellValue(item);
    if(target===null)continue;

    const desired=(item.untradeable===true||item.tradeable===false)?0:target;
    if(Number(item.discardValue)!==desired){
      item.discardValue=desired;
      changed++;
    }
  }
  if(changed)logger(`[quicksell-gold-rare] normalized owned cards=${changed} range=658-921`);
  return changed;
}
...
function makePlayerItem(card, targetState=state, pile=PILE_CLUB, untradeable=false) {
  ...
  return {
    ...
    training: 0,
    playStyle: 0,
    discardValue: playerQuickSellValue(card,{untradeable}),
    lastSalePrice: 0,
    marketDataMinPrice: 150,
    marketDataMaxPrice: 15000000,
    minPrice: 150,
...
const CONCEPT_ITEM_ID_BASE = 1500000000;
let CONCEPT_CARDS = null;
let CONCEPT_BY_ITEM_ID = null;

function ensureConceptCatalog() {
  if(CONCEPT_CARDS&&CONCEPT_BY_ITEM_ID)return;
  const unique=new Map();
  for(const group of [catalog?.base,catalog?.specials,catalog?.legends]){
    for(const card of Array.isArray(group)?group:[]){
      const resourceId=Number(card?.resourceId)||0;
      if(resourceId>0&&!unique.has(resourceId))unique.set(resourceId,card);
    }
  }
  CONCEPT_CARDS=Array.from(unique.values()).sort((a,b)=>
    (Number(b.rating)||0)-(Number(a.rating)||0) ||
    (Number(a.resourceId)||0)-(Number(b.resourceId)||0)
  );
  CONCEPT_BY_ITEM_ID=new Map();
  CONCEPT_CARDS.forEach((card,index)=>CONCEPT_BY_ITEM_ID.set(CONCEPT_ITEM_ID_BASE+index,card));
}

function isConceptItemId(itemId) {
  ensureConceptCatalog();
  return CONCEPT_BY_ITEM_ID.has(Number(itemId));
}

function conceptItemById(itemId) {
  ensureConceptCatalog();
  const numericId=Number(itemId)||0;
  const card=CONCEPT_BY_ITEM_ID.get(numericId);
  if(!card)return null;
  const item=makePlayerItem(card,{nextItemId:numericId},PILE_CLUB,true);
  item.id=numericId;
  item.itemState='concept';
  item.concept=true;
  item.isConcept=true;
  item.dream=true;
  item.owners=0;
...
function conceptItems(searchParams) {
  ensureConceptCatalog();
  let entries=CONCEPT_CARDS.map((card,index)=>({card,itemId:CONCEPT_ITEM_ID_BASE+index}));
  const position=String(searchParams.get('position')||'').toUpperCase();
  if(position&&position!=='ANY')entries=entries.filter(entry=>String(entry.card.position||'').toUpperCase()===position);
  for(const [parameter,field] of [['nation','nation'],['league','leagueId'],['team','teamId']]){
    if(!searchParams.has(parameter))continue;
    const value=Number(searchParams.get(parameter));
    if(Number.isFinite(value)&&value>=0)entries=entries.filter(entry=>Number(entry.card[field])===value);
  }
  const definitionId=Number(searchParams.get('defId')||searchParams.get('definitionId')||searchParams.get('resourceId')||0);
  if(definitionId>0)entries=entries.filter(entry=>[Number(entry.card.resourceId),Number(entry.card.definitionId),Number(entry.card.assetId)].includes(definitionId));
  const minRating=Number(searchParams.get('minRating')||0);
  const maxRating=Number(searchParams.get('maxRating')||0);
  if(minRating>0)entries=entries.filter(entry=>(Number(entry.card.rating)||0)>=minRating);
  if(maxRating>0)entries=entries.filter(entry=>(Number(entry.card.rating)||0)<=maxRating);
  const level=String(searchParams.get('level')||'').toLowerCase();
  if(level&&level!=='any')entries=entries.filter(entry=>{
    const rating=Number(entry.card.rating)||0;
    return level==='gold'?rating>=75:level==='silver'?rating>=65&&rating<=74:level==='bronze'?rating<=64:true;
  });
  const total=entries.length;
  const offset=Math.max(0,Number(searchParams.get('offset')||searchParams.get('start')||searchParams.get('skip')||0));
  const count=Math.max(1,Math.min(100,Number(searchParams.get('count')||searchParams.get('num')||50)));
  const items=entries.slice(offset,offset+count).map(entry=>conceptItemById(entry.itemId));
  return {item:items,items,itemData:items,total,totalResults:total,count:items.length,start:offset,offset,endOfList:offset+items.length>=total};
}

function dreamSquadPackList() {
  ensureConceptCatalog();
  const uniquePlayers=[];
  const seenAssets=new Set();
  for(const card of CONCEPT_CARDS){
    const assetId=Number(card.assetId)||0;
    if(assetId<=0||seenAssets.has(assetId))continue;
    seenAssets.add(assetId);
    uniquePlayers.push(card);
    if(uniquePlayers.length>=92)break;
  }
  const packs=[];
  for(let packIndex=0;packIndex<4;packIndex++){
    const cards=uniquePlayers.slice(packIndex*23,(packIndex+1)*23);
    if(cards.length<23)break;
    const attributes=attributeIndex=>cards.map(card=>Number(card.attributes?.[attributeIndex])||0);
    const squad=cards.map(card=>Number(card.assetId)||0);
    const ratings=cards.map(card=>Number(card.rating)||0);
    const rareFlags=cards.map(card=>Number(card.rareFlag)||1);
    const teamIds=cards.map(card=>Number(card.teamId)||0);
    const attribute1=attributes(0),attribute2=attributes(1),attribute3=attributes(2);
    const attribute4=attributes(3),attribute5=attributes(4),attribute6=attributes(5);
    packs.push({
      id:packIndex+1,packId:packIndex+1,image:packIndex+1,imageId:packIndex+1,
      name:`Equipe concept ${packIndex+1}`,label:`Equipe concept ${packIndex+1}`,
      description:'Equipe complete modifiable avec tous les joueurs FIFA 17',
      display:true,visible:true,available:true,formation:'f442',formationId:0,manager:0,squad,
      teamId:teamIds,teamIds,Rating:ratings,rating:ratings,Rare:rareFlags,rare:rareFlags,
      playStyle:cards.map(()=>0),
      ...
      kicktakers:squad.slice(0,6),squadActives:[1,1,1,1,1,1]
    });
  }
  return {packList:packs,dreamSquads:packs,packs,items:packs,count:packs.length,total:packs.length};
}
...
function squadDocument(id=state.activeSquadId) {
  ...
  for(let index=0;index<23;index++){
    const itemId=Number(squad.itemIds[index]||0);
    const item=byId.get(itemId)||conceptItemById(itemId);
  ...
  return {
    personaId:PERSONA_ID,id:squad.id,squadId:squad.id,squadName:squad.name,name:squad.name,
    ...
    newsquad:0,captain:0,kicktakers:[],tactics:[],dreamSquad:players.some(entry=>entry.itemData?.concept===true),custom:'',
    players,
    manager:(()=>{
...
function regularSquads() {
  return state.squads.filter(squad=>!isInternalGhostSquad(squad)&&Number(squad.id)>0&&Number(squad.id)<8000);
}

function nextRegularSquadId() {
  const used=new Set(regularSquads().map(squad=>Number(squad.id)).filter(Boolean));
  for(let id=1;id<8000;id++)if(!used.has(id))return id;
  return Math.max(1,...used)+1;
}

function normalizeSquadName(value,fallback='New Squad') {
  let name=String(value??'').replace(/[\u0000-\u001f\u007f]/g,' ').replace(/\s+/g,' ').trim();
  if(!name)name=String(fallback||'New Squad').trim()||'New Squad';
  return name.slice(0,15);
}

function squadNameExists(name,excludeId=0) {
  const wanted=String(name||'').trim().toLocaleLowerCase('en-US');
  return regularSquads().some(squad=>
    Number(squad.id)!==Number(excludeId) &&
    String(squad.name||'').trim().toLocaleLowerCase('en-US')===wanted
  );
}

function squadList() {
  return regularSquads().map(squad=>{
    const doc=squadDocument(squad.id);
    // FIFA 17 has used both the compact list object and several aliases in
    // different front-end controllers. Publish the complete harmless set.
    return {
      id:squad.id,
      squadId:squad.id,
      ...
      active:Number(squad.id)===Number(state.activeSquadId),
      changed:false,
      valid:true,
      newsquad:0,
      dreamSquad:Boolean(doc.dreamSquad)
    };
  });
}

function createSquad(body={},cloneSourceId=0) {
  ...
  const source=sourceId?state.squads.find(entry=>Number(entry.id)===sourceId):null;

  const requestedName=normalizeSquadName(
    root.squadName ?? root.name ?? root.newSquadName ?? root.teamName,
    `Squad ${regularSquads().length+1}`
  );
  if(squadNameExists(requestedName)){
    return {status:466,error:'NAME_EXISTS',reason:'NAME_EXISTS',squadName:requestedName};
  }

  const id=nextRegularSquadId();
  const squad={
    id,
    name:requestedName,
    formation:String(root.formation||source?.formation||FORMATION),
    itemIds:source&&Array.isArray(source.itemIds)?source.itemIds.slice(0,23):[],
    managerId:Number(source?.managerId||0)
  };

  // A normal "create squad" starts empty unless FIFA explicitly sent players
  // or used the clone endpoint.
  const entries=Array.isArray(root.players)?root.players:Array.isArray(root.itemData)?root.itemData:[];
  if(entries.length){
    const owned=new Set(clubItems().map(item=>Number(item.id)));
    squad.itemIds=entries.slice(0,23)
      .map(entry=>Number(entry?.itemData?.id||entry?.id||0))
      .map(itemId=>owned.has(itemId)||isConceptItemId(itemId)?itemId:0);
  }else if(!sourceId){
    squad.itemIds=[];
    squad.managerId=0;
  }
  ...
  const doc=squadDocument(id);
  logger(`[squad-v17] created id=${id} name="${squad.name}" clone=${sourceId||0}`);
  return {status:200,...doc,squad:doc};
}

function renameSquad(squadId,body={}) {
  const id=Number(squadId)||0;
  const squad=state.squads.find(entry=>Number(entry.id)===id&&!isInternalGhostSquad(entry));
  if(!squad)return {status:404,error:'SQUAD_NOT_FOUND',reason:'SQUAD_NOT_FOUND'};

  const root=body && (body.squad||body) || {};
  const requested=normalizeSquadName(
    root.squadName ?? root.name ?? root.newSquadName ?? root.teamName,
    squad.name
  );

  if(squadNameExists(requested,id)){
    return {status:466,error:'NAME_EXISTS',reason:'NAME_EXISTS',squadName:requested};
  }

  squad.name=requested;
  saveState();
  const doc=squadDocument(id);
  logger(`[squad-v17] renamed id=${id} name="${squad.name}"`);
  return {status:200,...doc,squad:doc};
}

function deleteRegularSquad(squadId) {
  const id=Number(squadId)||0;
  const normal=regularSquads();
  if(normal.length<=1)return {status:469,error:'DELETING_LAST_SQUAD',reason:'DELETING_LAST_SQUAD'};
  if(!normal.some(s=>Number(s.id)===id))return {status:404,error:'SQUAD_NOT_FOUND',reason:'SQUAD_NOT_FOUND'};

  state.squads=state.squads.filter(s=>Number(s.id)!==id);
  if(Number(state.activeSquadId)===id){
    state.activeSquadId=Number(regularSquads()[0]?.id)||1;
  }
  saveState();
  logger(`[squad-v17] deleted id=${id} active=${state.activeSquadId}`);
  return {status:200,success:true,activeSquadId:state.activeSquadId};
}
...
function saveSquad(body, routeId) {
  ...
  const requestedId=Number(routeId || root.id || root.squadId || state.activeSquadId) || Number(state.activeSquadId) || 1;

  // 9000-9999 are transient offline-season opponent IDs, never user-owned squads.
  // Return a wire-only document if the client echoes one back instead of polluting Mes ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipes.
  ...
  const requestedName=normalizeSquadName(root.squadName??root.name,squad.name);
  if(squadNameExists(requestedName,requestedId)){
    return {status:466,error:'NAME_EXISTS',reason:'NAME_EXISTS',squadName:requestedName};
  }
  squad.name=requestedName;
  squad.formation=String(root.formation||squad.formation||FORMATION);
  const entries=Array.isArray(root.players)?root.players:Array.isArray(root.itemData)?root.itemData:[];
  if(entries.length){
    const owned=new Set(clubItems().map(item=>item.id));
    squad.itemIds=entries.slice(0,23).map(entry=>Number(entry?.itemData?.id||entry?.id||0)).map(id=>owned.has(id)||isConceptItemId(id)?id:0);
  }
...
const MNG_PACK_RATING_PROFILES = {
  goldNonRare: [
    ...
    {min:78,max:78,weight:12},{min:79,max:79,weight:5}
  ],
  goldRareStandard: [
    {min:80,max:81,weight:84.85},{min:82,max:82,weight:8},{min:83,max:83,weight:4},
    {min:84,max:84,weight:1.7},{min:85,max:85,weight:0.8},{min:86,max:86,weight:0.4},
    {min:87,max:88,weight:0.20},{min:89,max:90,weight:0.04},{min:91,max:94,weight:0.01}
  ],
  goldRarePremium: [
    {min:80,max:81,weight:78.35},{min:82,max:82,weight:12},{min:83,max:83,weight:5.5},
    {min:84,max:84,weight:2.6},{min:85,max:85,weight:1.0},{min:86,max:86,weight:0.4},
    {min:87,max:88,weight:0.10},{min:89,max:90,weight:0.04},{min:91,max:94,weight:0.01}
  ],
  goldRareJumbo: [
    {min:80,max:81,weight:77.4},{min:82,max:82,weight:12.5},{min:83,max:83,weight:5.5},
    {min:84,max:84,weight:2.8},{min:85,max:85,weight:1.1},{min:86,max:86,weight:0.45},
    {min:87,max:88,weight:0.15},{min:89,max:90,weight:0.07},{min:91,max:94,weight:0.03}
  ],
  goldRareAllRare: [
    {min:80,max:81,weight:75.9},{min:82,max:82,weight:13},{min:83,max:83,weight:6},
    {min:84,max:84,weight:3.0},{min:85,max:85,weight:1.3},{min:86,max:86,weight:0.6},
    {min:87,max:88,weight:0.15},{min:89,max:90,weight:0.04},{min:91,max:94,weight:0.01}
  ],
  silverNonRare: [
    {min:65,max:65,weight:26},{min:66,max:66,weight:24},{min:67,max:67,weight:21},
    {min:68,max:68,weight:17},{min:69,max:69,weight:12}
  ],
  silverRare: [
    {min:70,max:70,weight:36},{min:71,max:71,weight:27},{min:72,max:72,weight:20},
    {min:73,max:73,weight:11},{min:74,max:74,weight:6}
  ],
  bronzeNonRare: [
    {min:0,max:55,weight:36},{min:56,max:56,weight:22},{min:57,max:57,weight:18},
    {min:58,max:58,weight:14},{min:59,max:59,weight:10}
  ],
  bronzeRare: [
    {min:60,max:60,weight:34},{min:61,max:61,weight:26},{min:62,max:62,weight:20},
    {min:63,max:63,weight:13},{min:64,max:64,weight:7}
  ]
};

function mngWeightedBand(profile) {
  const total=profile.reduce((sum,entry)=>sum+Math.max(0,Number(entry.weight)||0),0);
...
function mngRatingProfile(tier,rare,packId=0) {
  ...
  if(tier==='silver')return rare?MNG_PACK_RATING_PROFILES.silverRare:MNG_PACK_RATING_PROFILES.silverNonRare;
  return rare?MNG_PACK_RATING_PROFILES.bronzeRare:MNG_PACK_RATING_PROFILES.bronzeNonRare;
}

function weightedBase(tier, rare, used, packId=0) {
  const tierPool=Array.isArray(pools[tier])?pools[tier]:[];
  const matchesRarity=card=>rare?Number(card?.rareFlag??card?.rareflag??0)>0:Number(card?.rareFlag??card?.rareflag??0)===0;
  let candidates=tierPool.filter(card=>!used.has(card.resourceId)&&matchesRarity(card));
  if(!candidates.length)candidates=tierPool.filter(card=>matchesRarity(card));
  if(!candidates.length)candidates=tierPool.filter(card=>!used.has(card.resourceId));
  if(!candidates.length)candidates=tierPool;
...
const MNG_NORMAL_PACK_ICON_SLOT_CHANCE=0.0025; // 0.25% of a promo slot

function randomSpecial(used, contents=PACK_CONTENT_DEFAULTS, tier='') {
  const config=loadPackAdminConfig();
  const selected={...PACK_CONTENT_DEFAULTS,...contents};

  let specials=pools.specials.filter(card=>
    !used.has(card.resourceId) &&
    packSpecialAllowed(card,config) &&
    selected[specialContentCategory(card)]===true
  );

  // A gold store pack must not suddenly return a bronze/silver special merely
  // because the promo pool also contains one. Keep the advertised pack tier.
  const requestedTier=String(tier||'').toLowerCase();
  if(['bronze','silver','gold'].includes(requestedTier)){
    const tierMatches=specials.filter(card=>String(card?.quality||'').toLowerCase()===requestedTier);
    if(tierMatches.length)specials=tierMatches;
  }

  const legends=selected.legends&&config.events.legends!==false
    ?pools.legends.filter(card=>!used.has(card.resourceId)&&Number(card?.rareFlag??card?.rareflag)===12)
    :[];

  // Icons remain packable outside the dedicated LEGENDES pack, but no longer
  // compete 1:1 with every TOTW/OTW card. That old uniform pool made Icon odds
  // explode whenever local promo selection was enabled.
  if(legends.length && (!specials.length || Math.random()<MNG_NORMAL_PACK_ICON_SLOT_CHANCE)){
    return legends[Math.floor(Math.random()*legends.length)];
  }

  return specials.length?specials[Math.floor(Math.random()*specials.length)]:null;
}

function mngLegendPackPool() {
  // Resolve by resourceId after catalog initialization so every card comes from
  // the exact runtime definition currently exposed to FIFA 17.
  return MNG_LEGEND_PACK_RESOURCE_IDS
    .map(resourceId=>catalogByResource.get(Number(resourceId)))
    .filter(Boolean)
    .filter(card=>Number(card?.rareFlag??card?.rareflag)===12)
    .filter(card=>!SBC_EXCLUSIVE_RESOURCE_IDS.has(Number(card.resourceId)));
}

function randomLegendPack318(used) {
  const legends=mngLegendPackPool();
  const available=legends.filter(card=>!used.has(Number(card.resourceId)));
...
function openPack(packId, body={}, includeDisabled=false, reward=false) {
  ...
  const requestedId=wirePackId===65535?318:wirePackId;
  if(wirePackId===65535)logger('[pack-club] redirected legacy overflowing packId=65535 to ICONES packId=318');
  ...
  if(pack.clubEssentials){
    ...
    for(let slot=0;slot<pack.count;slot++){
      ...
      if(slot<pack.players){
        ...
        if(!card&&slot<guaranteedLegends)card=Number(pack.id)===318?randomLegendPack318(used):randomLegend(used,contents);
        else if(!card&&(slot<(guaranteedLegends+specialSlots)||specialOnly))card=randomSpecial(used,contents,pack.tier);
        if(!card&&contents.base)card=weightedBase(pack.tier,rare,used,pack.id);
        if(!card)card=randomLegend(used,{...contents,legends:true})||weightedBase(pack.tier,rare,used,pack.id);
  ...
  saveState();
  logger(`[pack-club] opened pack=${pack.id} odds=MNG_V3 localPlayers=${body.localPlayerOdds===true?1:0} ratings=${drawn.filter(item=>item.itemType==='player').map(item=>item.rating).join(',')} rarities=${drawn.filter(item=>item.itemType==='player').map(item=>Number(item.rareFlag??item.rareflag)||0).join(',')} cardTypes=${drawn.filter(item=>item.itemType==='player').map(item=>String(item.cardType||'base')).join(',')} types=${drawn.map(item=>item.itemType).join(',')}`);
...
function rewardPackDocument(entry) {
  ...
  if(Number(entry.rewardResourceId)>0){
    ...
    const rewardName=`RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC - ${playerName}`;
    return {
  ...
  if(packId===MNG_LIVE_PACK97_ID){
    const liveBase='/fut/mng/packs/97';
    const liveBackground=`${liveBase}/background.png`;
    const livePlayer=`${liveBase}/live.png`;
    return {
      id:Number(entry.id),purchasedPackId:Number(entry.id),packId,
      assetId:MNG_LIVE_PACK97_ASSET_ID,actionType:'OPENPACK',packType:'CARDPACK',type:'PACK',quantity:1,
      name:'MNG LIVE PACK 97',localizedName:'MNG LIVE PACK 97',packName:'MNG LIVE PACK 97',title:'MNG LIVE PACK 97',displayName:'MNG LIVE PACK 97',
      description:'Test visuel de pack charge depuis GitHub sans rebuild Frosty.',
      packContentInfo:{itemQuantity:1,goldQuantity:1,silverQuantity:0,bronzeQuantity:0,rareQuantity:1,contentType:'PLAYERS'},
      displayGroup:{priority:0,value:'mypacks'},displayGroupAssetId:MNG_LIVE_PACK97_ASSET_ID,
      displayGroupUseDefaultImage:false,useDefaultImage:false,isPremium:true,
      image:liveBackground,imageUrl:liveBackground,imagePath:liveBackground,
      packImage:liveBackground,packImageUrl:liveBackground,packImagePath:liveBackground,
      backgroundImage:liveBackground,backgroundImageUrl:liveBackground,backgroundImagePath:liveBackground,
      foregroundImage:livePlayer,foregroundImageUrl:livePlayer,foregroundImagePath:livePlayer,
      playerImage:livePlayer,playerImageUrl:livePlayer,playerImagePath:livePlayer,
      largeImage:liveBackground,largeImageUrl:liveBackground,largeImagePath:liveBackground,
      dealType:'REWARD',saleType:'NONE',state:'active',visible:1,sortPriority:0,
      coins:0,points:0,currencies:[],isUnopened:true,recovered:true,purchased:true
    };
  }
  if(packId===STARTER_TOTW_PACK_ID||packId===STARTER_OTW_PACK_ID){
    ...
    const rewardName=packId===STARTER_TOTW_PACK_ID?'Pack TOTW Garanti':'Pack OTW Garanti';
    const rewardDescription=packId===STARTER_TOTW_PACK_ID
      ?'Contient exactement 1 joueur TOTW garanti.'
      :'Contient exactement 1 joueur OTW garanti.';
    return {
      id:Number(entry.id),purchasedPackId:Number(entry.id),packId,
      assetId:(packId===STARTER_OTW_PACK_ID?99:98),
      actionType:'OPENPACK',packType:'CARDPACK',type:'PACK',quantity:1,

      name:rewardName,
      localizedName:rewardName,
      packName:rewardName,
      title:rewardName,
      displayName:rewardName,
      shortName:rewardName,
      packTitle:rewardName,

      description:rewardDescription,
      localizedDescription:rewardDescription,
      packDescription:rewardDescription,
  ...
  if(PACKS[packId]){
    ...
    return {
      ...
      name:pack.name,nameLoc:pack.name,description:`RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense : ${pack.name}`,descriptionLoc:`RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense : ${pack.name}`,
...
const MNG_SBC_PLAYER_PACK_META_V17 = new Map([
  ...
  [295002,{name:'Philipp Lahm [Loan]', description:'Joueur garanti : Philipp Lahm en prÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âªt 10 matchs.'}],
  [296001,{name:'Franck RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',   description:'Joueur garanti : Franck RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry.'}],
...
function unopenedRewardPacks() {
  // One-time experimental reward so the existing club can test slot 97.
  if(!state.mngLivePack97Granted){
    if(!Array.isArray(state.rewardPacks))state.rewardPacks=[];
    state.rewardPacks.push({id:1700312097,packId:MNG_LIVE_PACK97_ID,source:'MNG_LIVE_PACK97_TEST'});
    state.mngLivePack97Granted=true;
    saveState();
    logger('[online-pack97] granted one-time test pack id=1700312097 packId=317 assetId=97');
  }
...
// Release 2.1.43: pack 318 bypasses the removed cloud-store pack 314.
// PACK_POOL_EMPTY. Keep the cloud wallet authoritative, but debit the resolved
// pack price through the existing revision-protected wallet endpoint and let the
// local FIFA 17 catalog choose the guaranteed rarity-12 Icon.
async function authorizeCloudIconPack318(body={}) {
  return authorizeCloudPackPurchase(318,body);
}
...
async function authorizeCloudSbcCompletion(challengeId,itemIds) {
  const synced=await forceMngCloudClubSync('before-sbc-submit');
  if(!synced)logger(`[sbc-cloud] pre-submit sync skipped challenge=${challengeId} error=${MNG_CLOUD_CLUB_LAST_ERROR||'CLUB_SYNC_FAILED'}; cloud transaction remains authoritative`);
  const result=await cloudMatchRequest('/api/sbc/complete',{challengeId:Number(challengeId),itemIds:itemIds.map(Number)});
  const cloudError=String(result.error||'').toUpperCase();
  if(!result.ok&&!synced&&isUnauthorizedInventorySyncFailure()&&['SBC_REQUIRES_11_PLAYERS','SBC_ITEM_NOT_FOUND','ITEM_NOT_FOUND'].includes(cloudError)){
    const transactionId=`local-sbc-${Number(challengeId)}-${Date.now()}`;
    logger(`[sbc-cloud] local inventory fallback challenge=${challengeId} items=${itemIds.length} cloudError=${cloudError}`);
    setTimeout(()=>suppressRejectedClubSync(`sbc-${challengeId}-local-fallback`),0);
    return {
      ok:true,status:200,transactionId,localFallback:true,cloudFallbackReason:cloudError,
      rewards:[
        {kind:'challenge',transactionId:`${transactionId}-challenge`,resourceIds:[]},
        {kind:'set',transactionId:`${transactionId}-set`,resourceIds:[]}
      ]
    };
  }
...
async function authorizeCloudQuickSell(itemIds) {
  const ids=[...new Set((Array.isArray(itemIds)?itemIds:[itemIds]).map(Number).filter(id=>Number.isSafeInteger(id)&&id>0))];
  if(!ids.length)return {ok:true,status:200,itemData:[],items:[],discardValue:0,revision:MNG_CLOUD_CLUB_REVISION,profile:MNG_CLOUD_PROFILE};

  if(!await forceMngCloudClubSync('before-quicksell')){
    return {ok:false,status:503,error:'CLUB_SYNC_FAILED',clubSyncError:MNG_CLOUD_CLUB_LAST_ERROR||'CLUB_SYNC_FAILED'};
  }

  const makeBody=()=>({
    transactionId:crypto.randomUUID(),
    itemIds:ids,
    // Compatibility aliases. Older/newer Worker revisions have used
    // slightly different field names; the extra fields are harmless.
    itemId:ids,
    items:ids.map(id=>({id}))
  });

  let result=await cloudMatchRequest('/api/items/quicksell',makeBody());

  // A freshly opened pack can race the cloud club revision. If the Worker
  // says the item is missing/stale, force one exact club resync and retry.
  if(!result.ok && [404,409].includes(Number(result.status))){
    logger(`[quicksell] cloud rejected first attempt status=${result.status} error=${result.error||''}; forcing club resync`);
    MNG_CLOUD_LAST_CLUB_KEY='';
    if(await forceMngCloudClubSync('quicksell-retry')){
      result=await cloudMatchRequest('/api/items/quicksell',makeBody());
    }
  }
...
const MNG_MATCH_START_SESSION_MS=120000;

function offlineMatchRequestKey(body={},mode='season',tournamentId=0) {
  const squadId=Math.max(1,Number(body?.squadId)||Number(state.activeSquadId)||1);
  const opponentTeamId=Math.max(0,Number(body?.opponentTeamId||body?.teamId)||0);
  const round=mode==='tournament'
    ?Math.max(1,Number(ensureTournamentProgressState()?.[Number(tournamentId)]?.round)||1)
    :Math.max(1,Number(state.singlePlayerSeason?.gamesPlayed||0)+1);
  return [String(mode||'season'),Number(tournamentId)||0,squadId,opponentTeamId,round].join(':');
}

function activeMatchCanBeReused(active,body={},mode='season',tournamentId=0) {
  if(!active||typeof active!=='object'||active.completed===true||Number(active.id)<=0)return false;
  const age=Math.max(0,Date.now()-Math.max(0,Number(active.createdAt)||0));
  if(age>MNG_MATCH_START_SESSION_MS)return false;
  const expected=offlineMatchRequestKey(body,mode,tournamentId);
  const current=String(active.requestKey||'');
  if(current)return current===expected;
  return String(active.mode||'season')===String(mode||'season') &&
    Number(active.tournamentId||0)===Number(tournamentId||0);
}

function buildOfflineSeasonMatchDocument(body,active) {
  const season=state.singlePlayerSeason;
  const matchId=Number(active?.id)||0;
  const teamId=Math.max(1,Number(active?.teamId)||Number(body?.opponentTeamId||body?.teamId)||1);
  const userSquad=squadDocument(Number(active?.squadId||body?.squadId)||state.activeSquadId);
  ...
  return {valid:true,success:true,reportIdEnabled:false,matchId,id:matchId,opponentPersonaId:0,teamId,opponentTeamId:teamId,
    ...
    startDateTime:Math.floor(Number(active?.createdAt||Date.now())/1000),squad:userSquad,userSquad,opponentSquad};
}

async function closeDifferentActiveMatch(body={},mode='season',tournamentId=0) {
  const active=state.singlePlayerSeason?.activeMatch;
  if(!active||active.completed===true)return;
  if(activeMatchCanBeReused(active,body,mode,tournamentId))return;

  const token=String(active.cloudToken||'');
  if(token){
    const result=await cloudMatchRequest('/api/matches/finish',{
      token,goalsFor:0,goalsAgainst:0,endReason:'CANCELLED'
    });
    logger(`[mng-match-session] V111 cancel stale matchId=${Number(active.id)||0} oldMode=${active.mode||''} status=${result.status} error=${result.error||''}`);
  }else{
    logger(`[mng-match-session] V111 clear stale unauthorised matchId=${Number(active.id)||0} oldMode=${active.mode||''}`);
  }
  state.singlePlayerSeason.activeMatch=null;
  saveState();
}

async function renewTournamentRunForLaunch(tournamentId,progress,reason='stale-run') {
  const id=Math.max(1,Number(tournamentId)||1);
  const clientRunId=Date.now()*10+id;
  const authorization=await authorizeCloudTournamentEntry(id,clientRunId);
  if(!authorization.ok){
    logger(`[mng-match-session] V111 tournament run renew failed tournamentId=${id} reason=${reason} status=${authorization.status} error=${authorization.error||''}`);
    return {ok:false,status:authorization.status||503,error:authorization.error||'TOURNAMENT_ENTRY_REJECTED'};
  }

  progress.cloudRunToken=String(authorization.token||'');
  progress.clientRunId=clientRunId;
  progress.round=1;
  ...
  logger(`[mng-match-session] V111 tournament run renewed tournamentId=${id} reason=${reason} clientRunId=${clientRunId} token=${String(progress.cloudRunToken).slice(0,8)}`);
  return {ok:true,token:progress.cloudRunToken};
}

function secureSeasonActiveMatch(local,body={},mode='season',tournamentId=0) {
  if(!state.singlePlayerSeason || typeof state.singlePlayerSeason!=='object'){
    state.singlePlayerSeason=freshSinglePlayerSeason(10);
  }

  const matchId=Number(local?.matchId||local?.id)||0;
  if(matchId<=0)return null;

  let active=state.singlePlayerSeason.activeMatch;
  if(!active || typeof active!=='object' || Number(active.id)!==matchId){
    active={
      id:matchId,
      round:Math.max(1,Number(local?.round)||Number(state.singlePlayerSeason.gamesPlayed||0)+1),
      teamId:Math.max(1,Number(local?.opponentTeamId||local?.teamId||body?.opponentTeamId||body?.teamId)||1),
      completed:false,
      createdAt:Date.now(),
      mode:String(mode||'season'),
      tournamentId:Math.max(0,Number(tournamentId)||0),
      squadId:Math.max(1,Number(body?.squadId)||Number(state.activeSquadId)||1),
      requestKey:offlineMatchRequestKey(body,mode,tournamentId),
      cloudToken:''
    };
    state.singlePlayerSeason.activeMatch=active;
    logger(`[mng-match] V100 rebuilt activeMatch matchId=${matchId} mode=${active.mode} tournamentId=${active.tournamentId}`);
  }else{
    active.mode=String(mode||active.mode||'season');
    active.tournamentId=Math.max(0,Number(tournamentId)||Number(active.tournamentId)||0);
    active.squadId=Math.max(1,Number(active.squadId)||Number(body?.squadId)||Number(state.activeSquadId)||1);
    active.requestKey=String(active.requestKey||offlineMatchRequestKey(body,mode,tournamentId));
  }
  return active;
}

function clearSecureSeasonActiveMatch(matchId) {
  const active=state.singlePlayerSeason?.activeMatch;
  if(active && (!matchId || Number(active.id)===Number(matchId))){
    state.singlePlayerSeason.activeMatch=null;
    return true;
  }
  return false;
...
function isOfflineMatchReadyRequest(body={}) {
  if(!body||typeof body!=='object')return false;
  if(!Array.isArray(body.items)||body.items.length===0)return false;
  if(body.squadId!==undefined||body.seasonId!==undefined||body.tournamentId!==undefined)return false;

  const resultMarkers=[
    'minutesPlayed','minutes','matchMinutes','goals','goalsScored','goalsFor','homeGoals',
    'goalsAgainst','goalsConceded','awayGoals','result','completed','matchCompleted',
    'finished','dnf','didNotFinish','quit','abandoned','shotsOnTarget','passAccuracy',
    'possession','successfulTackles','matchData','myMatchStats','opponentMatchStats'
  ];
  return !resultMarkers.some(key=>body[key]!==undefined);
}

function offlineMatchReadyDocument(body={}) {
  const draft=ensureDraftState();
  const draftActive=draft?.activeMatch && draft.activeMatch.completed!==true ? draft.activeMatch : null;
  const seasonActive=state.singlePlayerSeason?.activeMatch;
  const active=draftActive||seasonActive;

  if(!active||active.completed===true||Number(active.id)<=0){
    logger('[mng-match-ready] V115 rejected: no active offline match');
    return {_status:409,code:'MATCH_NOT_ACTIVE'};
  }

  const starterIds=[];
  for(const row of Array.isArray(body.items)?body.items:[]){
    const id=Math.max(0,Number(row?.id||row?.itemId)||0);
    if(id>0&&!starterIds.includes(id))starterIds.push(id);
  }

  active.matchReadyItemIds=starterIds.slice(0,11);
  active.matchReadyAt=Date.now();
  active.ready=true;
  saveState();

  const isDraft=Boolean(draftActive);
  const squad=isDraft
    ?draftSquadDocument()
    :squadDocument(Number(active.squadId)||Number(state.activeSquadId)||1);
  const startDateTime=Math.floor(Math.max(0,Number(active.createdAt)||Date.now())/1000);

  logger(`[mng-match-ready] V115 matchId=${Number(active.id)||0} mode=${isDraft?'draft':(active.mode||'season')} tournamentId=${Number(active.tournamentId)||0} starters=${active.matchReadyItemIds.length} squadId=${Number(squad?.squadId||squad?.id)||0}`);

  // Retail/native MatchReady shape.
  return {
    squad,
    startDateTime
  };
...
async function startSecureMatch(body={}) {
  ...
  if(draftMode){
    const local=startDraftMatch(body);
    if(Number(local._status||200)>=400)return local;

    const active=state.offlineDraft?.activeMatch;
    if(!active)return {_status:500,code:'DRAFT_MATCH_STATE_LOST'};

    // Idempotent FIFA retry after a successful authorization.
    if(String(active.cloudToken||'')){
      active.cloudAuthorized=true;
      logger(`[draft-match] V115 duplicate POST reused cloud-authorized matchId=${local.matchId}`);
      return {...local,cloudAuthorized:true,reused:true};
    }

    const authorize=()=>cloudMatchRequest('/api/matches/start',{
      mode:'draft',
      clientMatchId:Number(local.matchId),
      round:Number(local.round)||1,
      divisionId:10,
      draftRunToken:String(state.offlineDraft?.cloudRunToken||''),
      squadItemIds:(local.userSquad?.players||[]).slice(0,11).map(entry=>Number(entry?.itemData?.id)||0)
    });

    let authorization=await authorize();

    // A transient duplicate/start race should retry the SAME unique ID once.
    if(!authorization.ok && Number(authorization.status)===409){
      logger(`[draft-match] V115 cloud 409 first attempt matchId=${local.matchId} draftId=${Number(state.offlineDraft?.draftId)||0} error=${authorization.error||''}; retry same id`);
      await new Promise(resolve=>setTimeout(resolve,350));
      authorization=await authorize();
    }

    if(!authorization.ok){
      // Do not destroy the drafted squad. Return it to READY so a retry from
      // FIFA uses the same Draft run and the new per-run unique match ID.
      state.offlineDraft.activeMatch=null;
      state.offlineDraft.status='READY';
      state.offlineDraft.state='READY';
      saveState();
      logger(`[draft-match] V115 cloud authorization failed status=${authorization.status} error=${authorization.error||''} matchId=${local.matchId} draftId=${Number(state.offlineDraft?.draftId)||0}`);
      return {_status:authorization.status||503,code:authorization.error||'MATCH_AUTHORIZATION_FAILED'};
    }

    const activeAfter=state.offlineDraft?.activeMatch;
    if(!activeAfter)return {_status:500,code:'DRAFT_MATCH_STATE_LOST'};
    activeAfter.cloudToken=String(authorization.token||'');
    activeAfter.cloudAuthorized=true;
    saveState();

    logger(`[mng-match] V115 cloud draft start authorized draftId=${Number(state.offlineDraft?.draftId)||0} matchId=${local.matchId} squadId=${local.squadId} token=${String(authorization.token||'').slice(0,8)}`);
  ...
  if(tournamentMode){
    ...
    const progress=ensureTournamentProgressState()[tournamentId] || (ensureTournamentProgressState()[tournamentId]={
      tournamentId,round:1,tournamentData:'',progressData:'',completed:false,won:false,updatedAt:Date.now()
    });

    // A lost/claimed/prize run cannot authorize another match. The diagnostic
    // 20260911-171748 showed tournament 4 reusing that terminal token six
    // times, returning 409 every time. Renew only when the player actually
    // presses Play again.
    if(progress.localFallback!==true && (progress.completed===true || progress.won===true || !String(progress.cloudRunToken||''))){
      const renewed=await renewTournamentRunForLaunch(tournamentId,progress,
        !String(progress.cloudRunToken||'')?'missing-token':'terminal-run');
      if(!renewed.ok){
        progress.localFallback=true;
        progress.localFallbackReason=String(renewed.error||'TOURNAMENT_CLOUD_UNAVAILABLE');
        progress.cloudRunToken='';
        progress.updatedAt=Date.now();
        logger(`[mng-match-session] V117 tournament entry cloud bypass tournamentId=${tournamentId} status=${renewed.status||0} error=${renewed.error||''}`);
      }
    }

    const tournamentClubPreflight=await prepareOfflineMatchClubSync('before-tournament-match');
    if(!tournamentClubPreflight.ok){
      progress.localFallback=true;
      progress.localFallbackReason=String(tournamentClubPreflight.error||'CLUB_SYNC_FAILED');
      logger(`[mng-match-session] V117 tournament club sync bypass tournamentId=${tournamentId} error=${progress.localFallbackReason}`);
    }

    state.activeMode='tournament';
    state.activeTournamentId=tournamentId;

    await closeDifferentActiveMatch(body,'tournament',tournamentId);
    const local=startSeasonMatch({...body,tournamentId});
    const active=secureSeasonActiveMatch(local,body,'tournament',tournamentId);
    if(!active)return {_status:500,code:'MATCH_STATE_CREATE_FAILED'};

    // V116: once Cloud's anti-early-forfeit limiter has rejected this LOCAL
    // Solo Cup, keep the rest of that run local.
    if(progress.localFallback===true){
      active.cloudToken='';
      active.cloudAuthorized=false;
      active.cloudFallback=true;
      active.cloudFallbackReason=String(progress.localFallbackReason||'MATCH_EARLY_FORFEIT_LIMIT');
      saveState();
      logger(`[mng-match-session] V116 tournament local fallback reused matchId=${local.matchId} tournamentId=${tournamentId} reason=${active.cloudFallbackReason}`);
      return {...local,cloudAuthorized:false,localFallback:true,tournamentId};
    }

    // Client retries of the same POST /match must be idempotent. Once a token
    // exists, return the exact same local match session instead of creating a
    // second cloud match.
    if(String(active.cloudToken||'')){
      active.cloudAuthorized=true;
      logger(`[mng-match-session] V111 tournament duplicate POST reused matchId=${local.matchId} tournamentId=${tournamentId}`);
      return {...local,cloudAuthorized:true,tournamentId,reused:true};
    }

    const makeAuthorization=()=>cloudMatchRequest('/api/matches/start',{
      mode:'tournament',
      clientMatchId:Number(local.matchId),
      round:Number(progress.round)||1,
      divisionId:10,
      tournamentRunToken:String(progress.cloudRunToken||''),
      squadItemIds:(local.userSquad?.players||[]).slice(0,11).map(entry=>Number(entry?.itemData?.id)||0)
    });

    let authorization=await makeAuthorization();

    if(!authorization.ok && Number(authorization.status)===409){
      const status=await cloudTournamentStatus(progress.cloudRunToken);
      const terminal=['lost','prize','claimed','completed'].includes(String(status?.status||'').toLowerCase());

      if(terminal){
        const renewed=await renewTournamentRunForLaunch(tournamentId,progress,`cloud-status-${String(status.status||'terminal')}`);
        if(renewed.ok)authorization=await makeAuthorization();
      }else{
        // Short transient duplicate/start race: retry the SAME match id once.
        await new Promise(resolve=>setTimeout(resolve,350));
        authorization=await makeAuthorization();
      }
    }

    if(!authorization.ok){
      const authError=String(authorization.error||'').toUpperCase();
      const fallbackReason=authError||`HTTP_${Number(authorization.status)||503}`;
      progress.localFallback=true;
      progress.localFallbackReason=fallbackReason;
      progress.cloudRunToken='';
      progress.updatedAt=Date.now();
      active.cloudToken='';
      active.cloudAuthorized=false;
      active.cloudFallback=true;
      active.cloudFallbackReason=fallbackReason;
      saveState();
      logger(`[mng-match-session] V117 tournament cloud bypass matchId=${local.matchId} tournamentId=${tournamentId} status=${authorization.status||0} error=${authorization.error||''}; continuing LOCAL`);
      return {...local,cloudAuthorized:false,localFallback:true,tournamentId};
    }

    const activeAfterAuth=secureSeasonActiveMatch(local,body,'tournament',tournamentId);
    if(!activeAfterAuth)return {_status:500,code:'MATCH_STATE_LOST'};

    activeAfterAuth.cloudToken=String(authorization.token||'');
    activeAfterAuth.cloudAuthorized=true;
    saveState();

    logger(`[mng-match] V111 cloud tournament start authorized matchId=${local.matchId} tournamentId=${tournamentId} token=${String(authorization.token||'').slice(0,8)}`);
  ...
  const seasonClubPreflight=await prepareOfflineMatchClubSync('before-season-match');
  if(!seasonClubPreflight.ok){
    logger(`[mng-match-session] V117 season club sync bypass error=${seasonClubPreflight.error||'CLUB_SYNC_FAILED'}; continuing LOCAL`);
  }

  await closeDifferentActiveMatch(body,'season',0);
  const local=startSeasonMatch({...body,tournamentId:0});
  const active=secureSeasonActiveMatch(local,body,'season',0);
  if(!active)return {_status:500,code:'MATCH_STATE_CREATE_FAILED'};

  if(String(active.cloudToken||'')){
    active.cloudAuthorized=true;
    logger(`[mng-match-session] V111 season duplicate POST reused matchId=${local.matchId}`);
    return {...local,cloudAuthorized:true,reused:true};
  }
  ...
  const makeAuthorization=()=>cloudMatchRequest('/api/matches/start',{
    mode:'season',
    clientMatchId:Number(local.matchId),
    round:Number(local.round)||Number(state.singlePlayerSeason?.gamesPlayed||0)+1,
    divisionId:Number(state.singlePlayerSeason?.divisionId)||10,
    squadItemIds:(local.userSquad?.players||[]).slice(0,11).map(entry=>Number(entry?.itemData?.id)||0)
  });
  ...
  if(!authorization.ok && Number(authorization.status)===409){
    // Same transient-start race protection as Solo Cup, but there is no
    // tournament run token to renew. Retry exactly the same clientMatchId.
    await new Promise(resolve=>setTimeout(resolve,350));
    authorization=await makeAuthorization();
  }
  ...
  if(!authorization.ok){
    active.cloudToken='';
    active.cloudFallback=true;
    active.cloudFallbackReason=String(authorization.error||`HTTP_${Number(authorization.status)||503}`);
    active.cloudAuthorized=false;
    saveState();
    logger(`[mng-match-session] V117 season cloud bypass matchId=${local.matchId} status=${authorization.status||0} error=${authorization.error||''}; continuing LOCAL`);
    return {...local,cloudAuthorized:false,localFallback:true};
  }

  const activeAfterAuth=secureSeasonActiveMatch(local,body,'season',0);
  if(!activeAfterAuth)return {_status:500,code:'MATCH_STATE_LOST'};
  ...
  logger(`[mng-match] V111 cloud season start authorized matchId=${local.matchId} token=${String(authorization.token||'').slice(0,8)}`);
  return {...local,cloudAuthorized:true};
}

function recordLocalTournamentFallbackMatch(body={},active={}) {
  const tournamentId=Math.max(1,Number(active?.tournamentId||state.activeTournamentId)||1);
  const definition=OFFLINE_TOURNAMENT_DEFS.find(entry=>Number(entry.id)===tournamentId) || OFFLINE_TOURNAMENT_DEFS[0];
  const progressMap=ensureTournamentProgressState();
  const progress=progressMap[tournamentId] || (progressMap[tournamentId]={
    tournamentId,round:Math.max(1,Number(active?.round)||1),tournamentData:'',progressData:'',
    completed:false,won:false,localFallback:true,localWins:0,updatedAt:Date.now()
  });
  ...
  if(isEarlyEmptyMatchFailure(body,active)){
    ...
    progress.updatedAt=Date.now();
    saveState();
    return {result:'CANCELLED',loadFailed:true,coinsAwarded:0,myScore:0,opponentScore:0,tournamentId};
  }

  const scores=scoresFromMatch(body);
  const result=scores.myScore>scores.opponentScore?'WIN':scores.myScore<scores.opponentScore?'LOSS':'DRAW';
  const round=Math.max(1,Math.min(4,Number(progress.round)||Number(active?.round)||1));
  const roundCoins=Math.max(0,Number(definition?.rounds?.[round-1]?.[1])||0);
  let coinsAwarded=roundCoins;

  progress.localFallback=true;
  progress.updatedAt=Date.now();

  if(result==='WIN'){
    progress.localWins=Math.max(Number(progress.localWins)||0,round-1)+1;
    progress.round=Math.min(5,Math.max(round+1,Number(progress.localWins)+1));

    if(Number(progress.localWins)>=4){
      progress.completed=true;
      progress.won=true;
      if(!progress.localRewardGranted){
        const prize=Math.max(0,Number(definition?.prize)||0);
        coinsAwarded+=prize;
        state.tournamentTrophies[tournamentId]=Math.max(0,Number(state.tournamentTrophies[tournamentId])||0)+1;

        const rewardResourceId=Math.max(0,Number(definition?.rewardResourceId)||0);
        if(rewardResourceId>0){
          const card=catalogByResource.get(rewardResourceId);
          if(card){
            const item=makePlayerItem(card,state,PILE_PURCHASED,true);
            item.cloudTransactionId=`LOCAL-TOURNAMENT-${tournamentId}-${Date.now()}`;
            state.items.push(item);
            state.pending.push(Number(item.id));
            progress.localRewardItemId=Number(item.id);
          }
        }
        progress.localRewardGranted=true;
      }
    }
  }else{
    progress.completed=true;
  ...
  state.history.unshift({
    ...
    type:'OFFLINE_TOURNAMENT_MATCH_LOCAL_FALLBACK',
    tournamentId,
    round,
    result,
    score:`${scores.myScore}-${scores.opponentScore}`,
    coins:coinsAwarded,
    reason:String(active?.cloudFallbackReason||'MATCH_EARLY_FORFEIT_LIMIT')
  });
  state.history=state.history.slice(0,200);
  saveState();

  logger(`[solo-cup] V116 local fallback finish tournamentId=${tournamentId} round=${round} result=${result} coins=${coinsAwarded} wins=${Number(progress.localWins)||0} completed=${Boolean(progress.completed)}`);
  return {result,coinsAwarded,seasonCoinsAwarded:0,tournamentId,...scores,localFallback:true};
}

async function finishSecureMatch(body={},explicitMatchId=0) {
  const draftMode=Boolean(state.offlineDraft?.activeMatch);
  const seasonActive=state.singlePlayerSeason?.activeMatch;
  const tournamentMode=!draftMode&&(
    String(seasonActive?.mode||'')==='tournament' ||
    Number(seasonActive?.tournamentId)>0 ||
    state.activeMode==='tournament'
  );
  const active=draftMode?state.offlineDraft.activeMatch:seasonActive;
  const token=String(active?.cloudToken||'');
  const submitted={...body};

  if(!token){
    if(!draftMode && tournamentMode && active?.cloudFallback===true){
      if(explicitMatchId>0)submitted.matchId=explicitMatchId;
      const outcome=recordLocalTournamentFallbackMatch(submitted,active);
      logger(`[mng-match-session] V116 local tournament finish matchId=${Number(active.id)||0} tournamentId=${Number(active.tournamentId)||0} result=${outcome.result||''} coins=${outcome.coinsAwarded||0}`);
      return {_status:200,...matchEndDocument(submitted,outcome,false),localFallback:true,tournamentId:Number(active.tournamentId)||0};
    }
    if(!draftMode && !tournamentMode && active?.cloudFallback===true){
      const outcome=recordSeasonMatch(submitted,null);
      logger(`[mng-match-session] V111 local season finish matchId=${Number(active.id)||0} result=${outcome.result||''} coins=${outcome.coinsAwarded||0}`);
      return {_status:200,...matchEndDocument(submitted,outcome,true),localFallback:true};
    }
    return {_status:409,code:'MATCH_NOT_CLOUD_AUTHORIZED'};
  }
  if(explicitMatchId>0)submitted.matchId=explicitMatchId;
  ...
  if(!authorization.ok && !draftMode){
    active.cloudToken='';
    active.cloudAuthorized=false;
    active.cloudFallback=true;
    active.cloudFallbackReason=String(authorization.error||`HTTP_${Number(authorization.status)||503}`);
    if(tournamentMode){
      const progress=ensureTournamentProgressState()[Math.max(1,Number(active.tournamentId||state.activeTournamentId)||1)];
      if(progress){
        progress.localFallback=true;
        progress.localFallbackReason=active.cloudFallbackReason;
        progress.cloudRunToken='';
      }
      const outcome=recordLocalTournamentFallbackMatch(submitted,active);
      logger(`[mng-match-session] V117 tournament finish cloud bypass matchId=${Number(active.id)||0} error=${authorization.error||''}`);
      return {_status:200,...matchEndDocument(submitted,outcome,false),localFallback:true,tournamentId:Number(active.tournamentId)||0};
    }
    const outcome=recordSeasonMatch(submitted,null);
    logger(`[mng-match-session] V117 season finish cloud bypass matchId=${Number(active.id)||0} error=${authorization.error||''}`);
    return {_status:200,...matchEndDocument(submitted,outcome,true),localFallback:true};
  }
  ...
  let outcome;
  if(tournamentMode){
    const tournamentId=Math.max(1,Number(authorization.tournament?.tournamentId||active?.tournamentId||state.activeTournamentId)||1);
...
async function forceMngCloudClubSync(reason='before-market-list') {
  ...
  if(MNG_CLOUD_CLUB_SYNC_IN_FLIGHT){
    MNG_CLOUD_CLUB_LAST_ERROR='CLUB_SYNC_BUSY';
    return false;
  }
  MNG_CLOUD_LAST_CLUB_KEY='';
  MNG_CLOUD_CLUB_LAST_ERROR='';
...
function isUnauthorizedInventorySyncFailure() {
  return String(MNG_CLOUD_CLUB_LAST_ERROR||'').toUpperCase()==='UNAUTHORIZED_INVENTORY_CHANGE';
}

function suppressRejectedClubSync(reason='local-fallback') {
  if(MNG_CLOUD_CLUB_SYNC_TIMER){
    clearTimeout(MNG_CLOUD_CLUB_SYNC_TIMER);
    MNG_CLOUD_CLUB_SYNC_TIMER=null;
  }
  MNG_CLOUD_CLUB_SYNC_PENDING=false;
  // This state is deliberately local-only because the currently deployed
  // cloud /api/club endpoint refuses the inventory delta. Do not hammer it.
  MNG_CLOUD_LAST_CLUB_KEY=cloudClubKey();
  logger(`[mng-cloud] V99 suppressed rejected club sync reason=${reason} key=${MNG_CLOUD_LAST_CLUB_KEY}`);
}

async function prepareOfflineMatchClubSync(reason='before-offline-match') {
  const synced=await forceMngCloudClubSync(reason);
  if(synced)return {ok:true,legacyClubSynced:true};

  // 2.1.2: opening packs / local special cards can legitimately make the
  // OLD /api/club inventory guard reject the complete local inventory with
  // UNAUTHORIZED_INVENTORY_CHANGE. That endpoint is still needed by the
  // market, but an OFFLINE match must never be blocked by that unrelated
  // inventory rejection.
  if(isUnauthorizedInventorySyncFailure()){
    const error=String(MNG_CLOUD_CLUB_LAST_ERROR||'UNAUTHORIZED_INVENTORY_CHANGE');
    suppressRejectedClubSync(`${reason}-offline-match-bypass`);
    queueMngFullSaveSync(`${reason}-legacy-club-rejected`,250);
    logger(`[mng-match-session] V112 legacy club sync bypass reason=${reason} error=${error}; offline match launch continues`);
    return {ok:true,legacyClubSynced:false,bypassed:true,error};
  }

  logger(`[mng-match-session] V112 club sync hard failure reason=${reason} error=${MNG_CLOUD_CLUB_LAST_ERROR||'CLUB_SYNC_FAILED'}`);
  return {ok:false,legacyClubSynced:false,error:MNG_CLOUD_CLUB_LAST_ERROR||'CLUB_SYNC_FAILED'};
}
...
function snapFutMarketPrice(value,minimum=150) {
  let price=Math.max(minimum,Math.floor(Number(value)||minimum));
  const step=price<=1000?50:price<=10000?100:price<=50000?250:price<=100000?500:1000;
  price=Math.max(minimum,Math.round(price/step)*step);
  return Math.min(15000000,price);
}

function normalizeCloudListingBody(body={},item=null) {
  const itemId=Number(body?.itemData?.id||body?.itemId||body?.id||item?.id||0);
  const resourceId=Number(item?.resourceId||body?.itemData?.resourceId||body?.resourceId||0);
  const startingBid=snapFutMarketPrice(body?.startingBid??body?.bid??150,150);
  const requestedBuyNow=body?.buyNowPrice??body?.buyNow??startingBid;
  const buyNowPrice=Math.max(startingBid,snapFutMarketPrice(requestedBuyNow,200));
  ...
  const duration=allowed.includes(requestedDuration)?
    requestedDuration:
    allowed.reduce((a,b)=>Math.abs(b-requestedDuration)<Math.abs(a-requestedDuration)?b:a,3600);

  return {
    ...body,
    itemData:{...(body?.itemData||{}),id:itemId},
    itemId,
    id:itemId,
    resourceId,
    startingBid,
    ...
    buyNow:buyNowPrice,
    duration,
    transactionId:String(body?.transactionId||body?.idempotencyKey||crypto.randomUUID())
  };
}

async function cloudListOwnedItem(body={}) {
  const itemId=Number(body?.itemData?.id||body?.itemId||body?.id||0);
  const item=state.items.find(entry=>Number(entry.id)===itemId);

  const performList=async candidate=>{
    const normalized=normalizeCloudListingBody(body,candidate);
    logger(`[mng-market] list request item=${normalized.itemId} resourceId=${normalized.resourceId} start=${normalized.startingBid} buyNow=${normalized.buyNowPrice} duration=${normalized.duration}`);
    return await mngCloudMarketRequest('/api/market/list',{method:'POST',body:normalized});
  };

  // Relisting an expired cloud-owned item may no longer be in local state.
  if(!item){
    const payload=await performList(null);
    if(!payload.ok){
      logger(`[mng-market] relist rejected item=${itemId} status=${payload.status} error=${payload.error||''}`);
      return {status:payload.status,error:payload.error,reason:payload.error};
    }
    MNG_CLOUD_MARKET_COUNTS.active++;
    MNG_CLOUD_MARKET_COUNTS.expired=Math.max(0,MNG_CLOUD_MARKET_COUNTS.expired-1);
    logger(`[mng-market] relisted expired item=${itemId} tradeId=${payload.tradeId} price=${payload.buyNowPrice}`);
    return {
      status:200,success:true,id:Number(payload.tradeId),tradeId:Number(payload.tradeId),
      tradeState:'active',buyNowPrice:Number(payload.buyNowPrice),startingBid:Number(payload.startingBid),
      currentBid:0,expires:Number(payload.expires)
    };
  }

  if(item.untradeable||item.tradeable===false){
    return {status:403,error:'ITEM_UNTRADEABLE',reason:'ITEM_UNTRADEABLE'};
  }

  if(!await forceMngCloudClubSync('before-market-list')){
    if(isUnauthorizedInventorySyncFailure()){
      logger(`[mng-market] V99 local fallback item=${itemId} because cloud club rejected inventory delta`);
      const local=listOwnedItem(body);
      if(Number(local?.status||200)<400){
        suppressRejectedClubSync('market-local-fallback');
        return {...local,success:true,localFallback:true,cloudFallbackReason:'UNAUTHORIZED_INVENTORY_CHANGE'};
      }
      return local;
    }
    return {status:503,error:'CLUB_SYNC_FAILED',reason:MNG_CLOUD_CLUB_LAST_ERROR||'CLUB_SYNC_FAILED'};
  }

  let payload=await performList(item);

  // New pack items can arrive immediately after a club revision change.
  // Retry once after forcing an exact cloud-club resync.
  if(!payload.ok && [404,409].includes(Number(payload.status))){
    logger(`[mng-market] list first attempt rejected item=${itemId} status=${payload.status} error=${payload.error||''}; forcing club resync`);
    MNG_CLOUD_LAST_CLUB_KEY='';
    if(await forceMngCloudClubSync('market-list-retry')){
      payload=await performList(item);
    }
  }

  if(!payload.ok){
    const cloudError=String(payload.error||'');
    const localEligible=String(item?.acquisitionSource||'').toUpperCase()==='CLOUD_PACK';
    if(localEligible && [400,403,404,409].includes(Number(payload.status))){
      logger(`[mng-market] V99 cloud list rejected local pack item=${itemId} status=${payload.status} error=${cloudError}; using local fallback`);
      const local=listOwnedItem(body);
      if(Number(local?.status||200)<400){
        suppressRejectedClubSync('market-cloud-reject-fallback');
        return {...local,success:true,localFallback:true,cloudFallbackReason:cloudError||`HTTP_${payload.status}`};
      }
    }
    logger(`[mng-market] list rejected item=${itemId} status=${payload.status} error=${cloudError}`);
    return {status:payload.status,error:payload.error,reason:payload.error};
  }

  state.items=state.items.filter(entry=>Number(entry.id)!==itemId);
  state.pending=state.pending.filter(id=>Number(id)!==itemId);
  state.listings=(state.listings||[]).filter(entry=>!(Number(entry.itemData?.id)===itemId&&entry.tradeState==='inactive'));
  for(const squad of state.squads||[]){
    if(Array.isArray(squad.itemIds))squad.itemIds=squad.itemIds.map(id=>Number(id)===itemId?0:id);
  }

  MNG_CLOUD_CLUB_REVISION=Number(payload.revision)||MNG_CLOUD_CLUB_REVISION;
  MNG_CLOUD_LAST_CLUB_KEY=cloudClubKey();
  MNG_CLOUD_MARKET_COUNTS.active++;
  MNG_CLOUD_MARKET_COUNTS.total++;
  saveState();

  logger(`[mng-market] listed item=${itemId} resourceId=${item.resourceId} tradeId=${payload.tradeId} price=${payload.buyNowPrice}`);
  return {
    status:200,success:true,id:Number(payload.tradeId),tradeId:Number(payload.tradeId),
    tradeState:'active',buyNowPrice:Number(payload.buyNowPrice),startingBid:Number(payload.startingBid),
    currentBid:0,expires:Number(payload.expires)
...
async function cloudBuyMarketListing(tradeId,body={}) {const transactionId=String(body.transactionId||body.idempotencyKey||crypto.randomUUID());const expectedPrice=Number(body.bid||body.buyNowPrice||body.buyNow||0);const status=await mngCloudMarketRequest('/api/market/status',{query:new URLSearchParams({ids:String(Number(tradeId)),includePublic:'1'})});const listing=status.auctionInfo?.[0];const isBid=listing&&expectedPrice<Number(listing.buyNowPrice);const payload=await mngCloudMarketRequest(isBid?'/api/market/bid':'/api/market/buy',{method:'POST',body:isBid?{listingId:Number(tradeId),transactionId,amount:expectedPrice}:{listingId:Number(tradeId),transactionId,expectedPrice}});if(!payload.ok)return {status:payload.status,error:payload.error,reason:payload.error,auctionInfo:[]};const item=isBid?null:payload.itemData;if(item&&!state.items.some(entry=>Number(entry.id)===Number(item.id))){state.items.push(item);state.pending.push(Number(item.id));}MNG_CLOUD_CLUB_REVISION=Number(payload.revision)||MNG_CLOUD_CLUB_REVISION;if(payload.profile){state.coins=Number(payload.profile.coins)||0;state.points=Number(payload.profile.fifaPoints)||0;MNG_CLOUD_PROFILE.coins=state.coins;MNG_CLOUD_PROFILE.fifaPoints=state.points;MNG_CLOUD_PROFILE.walletRevision=Number(payload.profile.walletRevision)||MNG_CLOUD_PROFILE.walletRevision;persistMngCloudSessionWallet(payload.profile);MNG_CLOUD_LAST_WALLET_KEY=mngWalletKey();}MNG_CLOUD_LAST_CLUB_KEY=cloudClubKey();saveState();logger(`[mng-market] ${isBid?'bid placed':'bought'} tradeId=${tradeId} amount=${expectedPrice}`);return {auctionInfo:payload.auctionInfo||[],itemData:item||undefined,items:item?[item]:[],duplicateItemIdList:item?duplicatePairs([item]):[],purchased:!isBid,newItem:!isBid,credits:state.coins,totalCredits:state.coins,coins:state.coins};}
async function cloudTradePile() {
  const payload=await mngCloudMarketRequest('/api/market/my');

  if(payload.ok&&payload.profile){
    state.coins=Number(payload.profile.coins)||0;
    state.points=Number(payload.profile.fifaPoints)||0;
    MNG_CLOUD_PROFILE.coins=state.coins;
    MNG_CLOUD_PROFILE.fifaPoints=state.points;
    MNG_CLOUD_PROFILE.walletRevision=Number(payload.profile.walletRevision)||MNG_CLOUD_PROFILE.walletRevision;
    persistMngCloudSessionWallet(payload.profile);
    MNG_CLOUD_LAST_WALLET_KEY=mngWalletKey();
  }

  if(payload.ok){
    const restoredIds=new Set((payload.restoredItemIds||[]).map(Number));
    if(restoredIds.size){
      let repaired=false;
      for(const item of state.items){
        // Do not overwrite an active local-fallback transfer listing.
        const localActive=(state.listings||[]).some(entry=>Number(entry.itemData?.id)===Number(item.id)&&entry.tradeState==='active');
        if(restoredIds.has(Number(item.id))&&Number(item.pile)===5&&!localActive){
          item.pile=PILE_CLUB;item.itemState='free';repaired=true;
        }
      }
      if(repaired)saveState();
    }
  ...
  // V99: the deployed cloud inventory can refuse freshly-opened local pack
  // instances. Those auctions remain fully usable in FIFA's local transfer pile.
  const localAuctions=tradePileDocument().auctionInfo||[];

  if(!payload.ok){
    const localCounts={
      active:localAuctions.filter(x=>x.tradeState==='active').length,
      sold:localAuctions.filter(x=>x.tradeState==='closed').length,
      expired:localAuctions.filter(x=>x.tradeState==='expired').length
    };
    MNG_CLOUD_MARKET_COUNTS={...localCounts,total:localAuctions.length};
    return {
      status:200,success:true,localFallback:true,
      auctionInfo:localAuctions,duplicateItemIdList:[],total:localAuctions.length,
      credits:state.coins,totalCredits:state.coins,coins:state.coins,points:state.points,fifaPoints:state.points
    };
  }

  const cloudAuctions=payload.auctionInfo||[];
  const cloudItemIds=new Set(cloudAuctions.map(entry=>Number(entry.itemData?.id)));
  const combined=[
    ...cloudAuctions,
    ...localAuctions.filter(entry=>!cloudItemIds.has(Number(entry.itemData?.id)))
  ];

  const active=combined.filter(x=>x.tradeState==='active').length;
  const sold=combined.filter(x=>x.tradeState==='closed').length;
  const expired=combined.filter(x=>x.tradeState==='expired').length;
  MNG_CLOUD_MARKET_COUNTS={active,sold,expired,total:combined.length};

  return {
    auctionInfo:combined,duplicateItemIdList:[],total:combined.length,
    credits:state.coins,totalCredits:state.coins,coins:state.coins,points:state.points,fifaPoints:state.points
  };
}

async function cloudTradePileCounts() {
  const pile=await cloudTradePile();
  const auctions=pile.auctionInfo||[];
  const active=auctions.filter(x=>x.tradeState==='active').length;
  const sold=auctions.filter(x=>x.tradeState==='closed').length;
  const expired=auctions.filter(x=>x.tradeState==='expired').length;
  return {
    active,sold,expired,
    tradePileCount:auctions.length,
    auctionCount:active,
    transferListCount:auctions.length
  };
}
async function cloudTradeStatus(tradeIds=[]) {
  const ids=[...new Set(tradeIds.map(Number).filter(id=>Number.isSafeInteger(id)&&id>0))];
  const localQuery=new URLSearchParams({tradeIds:ids.join(',')});
  const local=tradeStatusDocument(localQuery).auctionInfo||[];
  const payload=await mngCloudMarketRequest('/api/market/status',{query:new URLSearchParams({ids:ids.join(',')})});

  if(!payload.ok){
    return {
      status:200,success:true,localFallback:true,
      auctionInfo:local,duplicateItemIdList:[],total:local.length,
      credits:state.coins,totalCredits:state.coins
    };
  }

  const cloud=payload.auctionInfo||[];
  const cloudTradeIds=new Set(cloud.map(entry=>Number(entry.tradeId||entry.id)));
  const combined=[...cloud,...local.filter(entry=>!cloudTradeIds.has(Number(entry.tradeId||entry.id)))];
  return {
    auctionInfo:combined,duplicateItemIdList:[],total:combined.length,
    credits:state.coins,totalCredits:state.coins
  };
}
async function cloudWatchList() {const payload=await mngCloudMarketRequest('/api/market/watchlist');if(!payload.ok)return {status:payload.status,error:payload.error,auctionInfo:[],duplicateItemIdList:[],total:0};return {auctionInfo:payload.auctionInfo||[],duplicateItemIdList:[],total:Number(payload.total)||0,credits:state.coins,totalCredits:state.coins};}
async function cloudHubDocument() {
  const [globalMarket,tradePile,watchList]=await Promise.all([
    mngCloudMarketRequest('/api/market/count'),
    cloudTradePile(),
    cloudWatchList()
  ]);
  const own=tradePile.auctionInfo||[];
  const watched=watchList.auctionInfo||[];
  const active=own.filter(entry=>entry.tradeState==='active').length;
  const sold=own.filter(entry=>entry.tradeState==='closed').length;
  const expired=own.filter(entry=>entry.tradeState==='expired').length;
  const won=watched.filter(entry=>entry.tradeState==='closed'&&entry.bidState==='highest').length;
  const outbid=watched.filter(entry=>entry.bidState==='outbid').length;
  const hub=hubDocument();
  return {...hub,
    auctionCount:Number(globalMarket.activeListings)||0,activeAuctionCount:Number(globalMarket.activeListings)||0,liveAuctionCount:Number(globalMarket.activeListings)||0,
    tradePileCount:own.length,transferListCount:own.length,activeTradeCount:active,activeTransferCount:active,
    soldItemCount:sold,soldItems:sold,expiredItemCount:expired,
    watchListCount:watched.length,watchedItemCount:watched.length,transferTargetCount:watched.length,
    wonItemCount:won,wonItems:won,outbidItemCount:outbid,outbidItems:outbid,
    credits:state.coins,totalCredits:state.coins,coins:state.coins,points:state.points,fifaPoints:state.points
  };
}
...
async function openStorePack(body={}) {
  ...
  if(requested===65535){
    const overflowReward=state.rewardPacks.find(entry=>Number(entry.packId)>65535);
    if(overflowReward){
      logger(`[reward-pack] resolved FIFA17 overflow packId=65535 to instance=${overflowReward.id} packId=${overflowReward.packId} source=${overflowReward.source||''}`);
      return openRewardPack(overflowReward.id);
    }
  }
  ...
  if(requested===318){
    const legendPool=mngLegendPackPool();
    if(!legendPool.length)return {status:503,error:'LEGEND_PACK_LOCAL_POOL_EMPTY',reason:'LEGEND_PACK_LOCAL_POOL_EMPTY'};
    if(legendPool.length!==MNG_LEGEND_PACK_RESOURCE_IDS.length){
      logger(`[icon-pack-318] warning local rarity-12 pool=${legendPool.length}/${MNG_LEGEND_PACK_RESOURCE_IDS.length}`);
    }else{
      logger(`[icon-pack-318] local rarity-12 pool ready count=${legendPool.length}`);
    }
    // MNG_PACK318_LOCAL_ICON_V1
    const authorization=await authorizeCloudIconPack318(body);
    ...
    if(!icon||Number(icon.rareFlag??icon.rareflag)!==12||!MNG_LEGEND_PACK_RESOURCE_SET.has(Number(icon.resourceId))){
      logger(`[icon-pack-318] invalid local result resourceId=${Number(icon?.resourceId)||0} rareFlag=${Number(icon?.rareFlag??icon?.rareflag)||0}`);
      return {status:503,error:'LEGEND_PACK_LOCAL_RESULT_INVALID',reason:'LEGEND_PACK_LOCAL_RESULT_INVALID'};
    }
    logger(`[icon-pack-318] opened icon=${icon.name||icon.displayName||icon.resourceId} resourceId=${icon.resourceId} rareFlag=12`);
    return result;
  }
  ...
  // MNG PACK ODDS V3
  // Cloud remains authoritative for purchase/idempotency/wallet. The actual
  // FIFA 17 PLAYER draw is local so mng-pack-admin.json + rating profiles are
  // really respected. V2 accidentally passed localPlayerOdds:false here.
  return openPack(requested,{
    ...body,
    cloudAuthorized:true,
    localPlayerOdds:false,
    cloudProfile:authorization.profile,
    cloudResourceIds:authorization.playerResourceIds,
    cloudNonPlayerResourceIds:authorization.nonPlayerResourceIds,
...
async function openRewardPack(instanceId) {
  ...
  if(index<0)return {status:404,error:'REWARD_PACK_NOT_FOUND',reason:'Aucun pack rÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense disponible.'};
  ...
  if(Array.isArray(entry.cloudResourceIds)&&entry.cloudResourceIds.length){
    ...
    if(cards.length!==entry.cloudResourceIds.length)return {status:409,error:'CLOUD_SBC_REWARD_INVALID',reason:'RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense DCE cloud introuvable.'};
  ...
  if(Number(entry.rewardResourceId)>0){
    ...
    if(!card)return {status:409,error:'SBC_PLAYER_REWARD_NOT_FOUND',reason:'Carte de rÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC introuvable.'};
    const item=makePlayerItem(card,state,PILE_PURCHASED,true);
    const loanGames=Math.max(0,Number(entry.loanGames)||0);
    if(loanGames>0){
...
async function discardItem(itemId) {
  ...
  if(!item){
    const ghost={id:numericId};
    return {itemData:ghost,items:[ghost],itemIds:[numericId],credits:state.coins,totalCredits:state.coins,coins:state.coins};
  }
  if(item.itemState==='discarded'||Number(item.pile)===0){
    state.pending=state.pending.filter(id=>Number(id)!==Number(item.id));
    saveState();
    const discarded={id:item.id,pile:0,itemState:'discarded',discardValue:0};
    return {itemData:discarded,items:[discarded],itemIds:[item.id],credits:state.coins,totalCredits:state.coins,coins:state.coins};
  }
  const result=await discardItems([item.id]);
  if(result._status)return result;
  const discarded=result.items?.[0]||result.itemData?.[0]||{id:item.id};
  return {
    itemData:discarded,
    items:[discarded],
    itemIds:[Number(discarded.id)||item.id],
    credits:state.coins,totalCredits:state.coins,coins:state.coins,
    discardValue:Number(result.discardValue)||0
  };
}

function localQuickSellFallback(owned,ids,reason='UNAUTHORIZED_INVENTORY_CHANGE') {
  const discarded=(Array.isArray(owned)?owned:[]).map(item=>({
    ...item,
    pile:0,
    itemState:'discarded',
    discardValue:Math.max(0,Math.floor(Number(item.discardValue)||0))
  }));
  const discardedIds=new Set(discarded.map(item=>Number(item.id)).filter(Boolean));
  const total=discarded.reduce((sum,item)=>sum+(Number(item.discardValue)||0),0);
  ...
  for(const squad of state.squads||[]){
    if(Array.isArray(squad.itemIds))squad.itemIds=squad.itemIds.map(id=>discardedIds.has(Number(id))?0:id);
  }
  state.listings=(state.listings||[]).filter(entry=>!discardedIds.has(Number(entry.itemData?.id)));

  state.coins=Math.max(0,Math.floor(Number(state.coins)||0))+total;
  if(MNG_CLOUD_PROFILE){
    MNG_CLOUD_PROFILE.coins=state.coins;
    MNG_CLOUD_PROFILE.fifaPoints=state.points;
  }
  ...
  state.history.unshift({
    time:new Date().toISOString(),type:'QUICK_SELL_LOCAL_FALLBACK',
    itemIds:[...discardedIds],discardValue:total,reason
  });
  state.history=state.history.slice(0,200);

  // Mark the inventory state local-only BEFORE saveState queues /api/club.
  MNG_CLOUD_LAST_CLUB_KEY=cloudClubKey();
  saveState();
  suppressRejectedClubSync('quicksell-local-fallback');

  logger(`[quicksell] V99 local fallback ids=${[...discardedIds].join(',')} value=${total} credits=${state.coins} reason=${reason}`);
  return {
    itemId:ids,
    itemIds:[...discardedIds],
    itemData:discarded,
    items:discarded,
    discardedItemIdList:[...discardedIds],
    discardValue:total,
    credits:state.coins,totalCredits:state.coins,coins:state.coins,
    localFallback:true,cloudFallbackReason:reason
  };
}
...
async function discardItems(itemIds) {
  ...
  // Make the requested 658-921 rare-gold value authoritative even for cards
  // that were obtained before this release.
  for(const item of owned){
    const rareGoldValue=goldRareQuickSellValue(item);
    if(rareGoldValue!==null){
      item.discardValue=(item.untradeable===true||item.tradeable===false)?0:rareGoldValue;
    }
  }

  if(!owned.length){
    return {
      itemId:ids,itemIds:ids,itemData:[],items:[],discardedItemIdList:[],
      discardValue:0,credits:state.coins,totalCredits:state.coins,coins:state.coins
    };
  }

  const coinsBeforeAuthorization=Math.max(0,Math.floor(Number(state.coins)||0));
  const authorization=await authorizeCloudQuickSell(owned.map(item=>item.id));
  if(!authorization.ok){
    if(
      authorization.error==='CLUB_SYNC_FAILED' &&
      isUnauthorizedInventorySyncFailure()
    ){
      return localQuickSellFallback(owned,ids,'UNAUTHORIZED_INVENTORY_CHANGE');
    }

    // If the cloud knows the account but does not know a freshly-opened local
    // pack instance, do not kick the user out of FUT; settle that instance
    // locally and let the normal wallet-sync path publish the new balance.
    const allLocalPackItems=owned.every(item=>String(item?.acquisitionSource||'').toUpperCase()==='CLOUD_PACK');
    if(allLocalPackItems && [400,403,404,409].includes(Number(authorization.status))){
      return localQuickSellFallback(owned,ids,String(authorization.error||`HTTP_${authorization.status}`));
    }

    return {
      _status:authorization.status||503,
      code:authorization.error||'QUICKSELL_CLOUD_REJECTED',
      reason:authorization.error||'QUICKSELL_CLOUD_REJECTED',
      itemData:[],items:[],itemIds:ids,
      credits:state.coins,totalCredits:state.coins,coins:state.coins
    };
  }

  // Worker compatibility: accept either itemData[] or items[].
  const cloudItems=Array.isArray(authorization.itemData)?authorization.itemData:
                   Array.isArray(authorization.items)?authorization.items:[];
  const valuesById=new Map(cloudItems.map(entry=>[Number(entry.id),Number(entry.discardValue)||0]));
  const discarded=owned.map(item=>{
    const rareGoldValue=goldRareQuickSellValue(item);
    const desiredValue=rareGoldValue!==null
      ?((item.untradeable===true||item.tradeable===false)?0:rareGoldValue)
      :(valuesById.has(Number(item.id))?valuesById.get(Number(item.id)):(Number(item.discardValue)||0));
    return {
      ...item,
      pile:0,
      itemState:'discarded',
      discardValue:Math.max(0,Math.floor(Number(desiredValue)||0))
    };
  });
  ...
  const localDiscardTotal=discarded.reduce((sum,item)=>sum+(Number(item.discardValue)||0),0);

  // /api/items/quicksell may still run an older cloud discard table. It has
  // already updated state.coins via applyCloudWalletProfile(). Reconcile the
  // difference so the player really receives the value displayed by FIFA.
  const cloudAward=Math.max(0,Math.floor(Number(state.coins)||0)-coinsBeforeAuthorization);
  const awardAdjustment=localDiscardTotal-cloudAward;
  if(awardAdjustment!==0){
    state.coins=Math.max(0,Math.floor(Number(state.coins)||0)+awardAdjustment);
    if(MNG_CLOUD_PROFILE){
      MNG_CLOUD_PROFILE.coins=state.coins;
      MNG_CLOUD_PROFILE.fifaPoints=state.points;
    }
    logger(`[quicksell-gold-rare] wallet reconcile desired=${localDiscardTotal} cloudAward=${cloudAward} adjustment=${awardAdjustment} coins=${state.coins}`);
    // saveState above may have happened before this adjustment. Persist and
    // queue the normal revision-aware /api/wallet/sync once more.
    saveState();
  }

  const discardValue=localDiscardTotal;

  // FIFA 17/18 ItemDAO reads response.items[].id after quick-sell.
  return {
    itemId:ids,
    itemIds:[...discardedIds],
    itemData:discarded,
...
function filteredClub(searchParams) {
  ...
  const responseItems=items.map(item=>{
    ...
    if(itemType==='player'){
      const specialValue=totwQuickSellValue(item);
      const rareGoldValue=goldRareQuickSellValue(item);
      if(specialValue!==null)item.discardValue=specialValue;
      else if(rareGoldValue!==null)item.discardValue=(item.untradeable===true||item.tradeable===false)?0:rareGoldValue;
    }
...
function wireTradeListing(listing) {
  const inactive=listing.tradeState===FIFA_TRADE_STATE_INACTIVE;
  ...
  return {
    ...
    currentBid:inactive?0:Number(listing.currentBid)||0,
    offers:inactive?0:Number(listing.offers)||0,
    expires:inactive?-1:Number(listing.expires)||0,
    tradeOwner:true
  };
}
...
function tradePileDocument() {
  ...
  const auctionInfo=(state.listings||[]).filter(entry=>entry.tradeState!=='closed').map(wireTradeListing);
  saveState();
...
function tradeStatusDocument(query) {
  ...
  return {auctionInfo:listings.map(wireTradeListing),duplicateItemIdList:[],total:listings.length,credits:state.coins,totalCredits:state.coins};
}
...
function auroraSbcElgReq(requirements) {
  ...
  for(const req of requirements||[]){
    ...
    if(type==='PLAYER_COUNT'){
      // Le nombre total de joueurs est portÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© par challenge.numPlayers/squadSize.
      // Ne jamais crÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©er un slot elgReq key=2 isolÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©.
      continue;
    }
...
function onlineSeasonDefinition(divisionId) {
  const division=Math.max(1,Math.min(10,Number(divisionId)||10));
  const seasonId=seasonIdForDivision(division);
  const thresholds=seasonThresholds(division);
  const championshipCoins=1900+(10-division)*900;
  const promotionCoins=division===10?1500:Math.max(500,championshipCoins-400);
  const maintenanceCoins=division===10?300:Math.max(300,Math.floor(championshipCoins/5));
  const relegationCoins=Math.max(250,Math.floor(championshipCoins/10));
  return {id:division,divisionId:seasonId,type:'ONLINE',numMatches:SEASON_MATCH_COUNT,
    matchLengthMin:6,matches:[],elgOperation:'AND',elgReq:[],prizeSet:[
      seasonPrize('RELEGATION',0,relegationCoins),seasonPrize('MAINTENANCE',thresholds.hold,maintenanceCoins),
      seasonPrize('PROMOTION',thresholds.promotion,promotionCoins),seasonPrize('CHAMPIONSHIP',thresholds.title,championshipCoins)],
    startDateTime:0,endDateTime:2147483647,untilStartSeconds:0,untilEndSeconds:315360000};
...
function seasonListDocument(searchParams) {
  const requestedType=String(searchParams?.get?.('type')||'offline').toLowerCase();
  const online=requestedType==='online';
  const seasons=Array.from({length:10},(_,index)=>online?onlineSeasonDefinition(10-index):seasonDefinition(10-index));
  logger(`[season-list] type=${online?'ONLINE':'OFFLINE'} count=${seasons.length}`);
  return {seasons};
}
...
function updateSeasonData(body) {
  ...
  if(Number.isFinite(Number(body?.divisionId))&&Number(body.divisionId)>0)season.divisionId=Math.max(1,Math.min(10,Number(body.divisionId)));
...
function seasonHistoryDocument(searchParams) {
  const season=state.singlePlayerSeason,totals=season.totals||{},includeCurrent=season.completed?0:1;
  const requestedType=String(searchParams?.get?.('type')||'offline').toLowerCase();
  return {type:requestedType==='online'?'online':'offline',bestPointsSeasonId:Number(totals.bestDivision)||season.divisionId,
...
function seasonLeaderboardOptionsDocument() {
  return {leaderboardInfo:[
    {id:1,name:'Saisons en ligne',group:'seasons',type:'ONLINE',period:'ALL_TIME',rankBy:'POINTS'}
  ],leaderboards:[
    {id:1,name:'Saisons en ligne',group:'seasons',type:'ONLINE',period:'ALL_TIME',rankBy:'POINTS'}
  ],entries:[]};
}
...
function offlineSeasonOpponentDocument(teamId,gameIndex=0) {
  ...
  const teamName=primaryPool[0]?.teamName||primaryPool[0]?.clubName||`ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°quipe IA ${teamId}`;
  return {
...
function startSeasonMatch(body) {
  ...
  const tournamentId=Math.max(0,Number(body?.tournamentId)||0);
  const requestedMode=tournamentId>0||state.activeMode==='tournament'?'tournament':'season';
  const requestKey=offlineMatchRequestKey(body,requestedMode,tournamentId);

  if(activeMatchCanBeReused(season.activeMatch,body,requestedMode,tournamentId)){
    const active=season.activeMatch;
    logger(`[mng-match-session] V111 reuse launch session matchId=${active.id} mode=${requestedMode} tournamentId=${tournamentId} token=${String(active.cloudToken||'').slice(0,8)}`);
    return buildOfflineSeasonMatchDocument(body,active);
  }

  season.started=true;
  const fixture=seasonDefinition(season.divisionId).matches[Math.min(season.gamesPlayed,SEASON_MATCH_COUNT-1)];
  const matchId=Number(season.nextMatchId++);
  const teamId=Number(body?.opponentTeamId||body?.teamId||fixture.teamId)||fixture.teamId;

  season.activeMatch={
    id:matchId,
    round:season.gamesPlayed+1,
    teamId,
    squadId:Math.max(1,Number(body?.squadId)||Number(state.activeSquadId)||1),
    completed:false,
    createdAt:Date.now(),
  ...
  const document=buildOfflineSeasonMatchDocument(body,season.activeMatch);
  logger(`[offline-season] matchId=${matchId} userSquad=${document.userSquadId} opponentSquad=${document.opponentSquadId} teamId=${teamId} players=${document.userSquad?.players?.length||0}`);
  return document;
}
...
function draftClientMatchId(draft,round=1) {
  const draftId=Math.max(1,Number(draft?.draftId)||1);
  const safeRound=Math.max(1,Math.min(4,Number(round)||1));

  // V115: the old code always generated 1717000001 for round 1 of EVERY
  // Draft run. Cloud keeps match idempotency/history, so a later run could
  // collide with an earlier run and return HTTP 409.
  //
  // Keep the value safely inside signed 32-bit range while making it unique
  // for 100,000 Draft runs x 4 rounds.
  return 1717000000 + ((draftId % 100000) * 4) + safeRound;
}

function buildDraftMatchDocument(active,draft=ensureDraftState()) {
  const opponentSquad=draftOpponentDocument();
  const userSquad=draftSquadDocument();
  const teamId=Math.max(1,Number(active?.teamId)||Number(opponentSquad?.teamId)||DRAFT_OPPONENT_TEAMS[0].teamId);
  const matchId=Math.max(1,Number(active?.id)||draftClientMatchId(draft,Number(draft?.wins||0)+1));
  const round=Math.max(1,Number(active?.round)||Number(draft?.wins||0)+1);
  ...
  return {
    ...
    startDateTime:Math.floor(Math.max(0,Number(active?.createdAt)||Date.now())/1000),
    opponentPersonaId:0,
    opponentSquadId:Number(opponentSquad?.squadId||opponentSquad?.id)||0,
    opponentTeamId:teamId,
    teamId,
    teamName:opponentSquad?.teamName||'',
    teamAbbr:opponentSquad?.teamAbbr||'',
    clubId:teamId,
    badgeId:teamId,
    difficulty:Number(draft?.difficulty)||2,
    items:[],
    squadId:Number(userSquad?.squadId||userSquad?.id)||900001,
    userSquadId:Number(userSquad?.squadId||userSquad?.id)||900001,
    squad:userSquad,
    userSquad,
    opponentSquad
  };
...
function startDraftMatch(body={}) {
  const draft=ensureDraftState();
  if(!draft||!['READY','PLAYING'].includes(String(draft.status)))return {_status:409,code:'DRAFT_NOT_READY'};

  state.activeMode='draft';

  // FIFA can retry POST /match. Reuse the exact active Draft match instead
  // of recreating it or allocating another cloud session.
  const existing=draft.activeMatch;
  if(existing && existing.completed!==true){
    const age=Math.max(0,Date.now()-Math.max(0,Number(existing.createdAt)||0));
    if(age<=MNG_MATCH_START_SESSION_MS){
      logger(`[draft-match] V115 reuse matchId=${Number(existing.id)||0} draftId=${Number(draft.draftId)||0} round=${Number(existing.round)||1}`);
      return buildDraftMatchDocument(existing,draft);
    }
  }

  const round=Math.max(1,Math.min(4,Number(draft.wins||0)+1));
  const matchId=draftClientMatchId(draft,round);
  const opponentSquad=draftOpponentDocument();
  ...
  const document=buildDraftMatchDocument(draft.activeMatch,draft);
  logger(`[draft-match] V115 round=${round} draftId=${Number(draft.draftId)||0} matchId=${matchId} squad=${document.squadId} userSquad=${document.userSquadId} opponentSquad=${document.opponentSquadId} teamId=${teamId} team=${document.teamName} players=${document.opponentSquad?.players?.filter(entry=>Number(entry?.itemData?.id)).length||0}`);
  return document;
}
...
const OFFLINE_TOURNAMENT_DEFS = [
  {id:1,name:'Coupe Bronze hors ligne',description:'Remportez quatre matchs ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â  ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©limination directe.',prize:500,trophyResourceId:1500,rounds:[[1,150],[1,200],[2,300],[2,500]]},
  {id:2,name:'Coupe Argent hors ligne',description:'Une coupe relevÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©e rÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©servÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©e au mode solo.',prize:750,trophyResourceId:1501,rounds:[[1,175],[2,250],[2,375],[3,650]]},
  {id:3,name:'Coupe Or hors ligne',description:'Affrontez des ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipes fortes et gagnez la finale.',prize:1000,trophyResourceId:1502,rounds:[[2,225],[2,325],[3,500],[3,850]]},
...
function tournamentProgressIsResumable(entry) {
  ...
  if(entry.nativeAdvertised===true)return true;
...
function tournamentUserListDocument() {
  ...
  const ids=Object.entries(progress)
    // V114: a cloudRunToken is internal MNG server state, NOT native FIFA
    // tournament progress. Only advertise a tournament once FIFA has supplied
    // real resumable bracket/progress data.
...
function tournamentUserDocument(tournamentId) {
  ...
  if(!tournamentProgressIsResumable(entry)){
    if(entry && String(entry.cloudRunToken||'')){
      logger(`[solo-cup] V114 hide cloud-only run tournamentId=${id} token=${String(entry.cloudRunToken).slice(0,8)} nativeProgress=empty`);
    }
    return {tournamentId:id,TOURNAMENT_ID:id,round:1,ROUND:1,active:false};
  }
...
async function updateTournamentUser(tournamentId,body) {
  ...
  const payload={
    ...
    completed:false,
    won:false,
    nativeAdvertised:String(document.tournamentData||'').trim().length>0
  };

  const progress=ensureTournamentProgressState();
  const previous=progress[id]||{};

  if(previous.localFallback===true){
    payload.clientRunId=Number(previous.clientRunId)||0;
    payload.cloudRunToken='';
    payload.localFallback=true;
    payload.localFallbackReason=String(previous.localFallbackReason||'MATCH_EARLY_FORFEIT_LIMIT');
    payload.localWins=Math.max(0,Number(previous.localWins)||0);
    payload.completed=Boolean(previous.completed);
    payload.won=Boolean(previous.won);
    if(Number(previous.round)>Number(payload.round))payload.round=Number(previous.round);
    state.activeTournamentId=id;
    progress[id]={...previous,...payload,updatedAt:Date.now()};
    saveState();
    logger(`[solo-cup] V116 local fallback progress tournamentId=${id} round=${payload.round} completed=${payload.completed}`);
  ...
  const cloudState=String(cloudStatus.status||'').toLowerCase();
  const clientRound=Math.max(1,Number(document.round)||1);
  const previousRound=Math.max(1,Number(previous.round)||1);
  const cloudRound=Math.max(1,Number(cloudStatus.wins||0)+1);
  payload.round=Math.min(5,Math.max(clientRound,previousRound,cloudRound));

  // FIFA writes the native bracket immediately after an accepted match end.
  // A delayed/stale Cloud "lost" status must not roll that persisted round
  // back to 1, otherwise leaving and reopening the Cup makes FIFA delete it.
  const preserveNativeProgress=cloudState==='lost' && previous.completed!==true &&
    previous.won!==true && Math.max(clientRound,previousRound)>1;
  if(preserveNativeProgress){
    payload.completed=false;
    payload.won=false;
    logger(`[solo-cup] V118 preserved native progress tournamentId=${id} clientRound=${clientRound} previousRound=${previousRound} cloudState=${cloudState}`);
  }else{
    payload.completed=['prize','claimed','lost'].includes(cloudState);
    payload.won=['prize','claimed'].includes(cloudState);
  }

  // A first-round bracket has all-zero progress bytes before the first match,
  // but it is still a real native Cup once FIFA supplied tournamentData.
  // Keep advertising it so leaving and reopening the menu can resume it.
  if(tournamentProgressIsResumable(payload)){
    ...
    progress[id]={
      ...
      progressData:'',cloudRunToken,clientRunId:payload.clientRunId,
      completed:Boolean(payload.completed),won:Boolean(payload.won),updatedAt:Date.now()
    };
  }
  saveState();
  return {...payload,TOURNAMENT_ID:id,ROUND:payload.round,success:true,valid:true};
...
function resetTournamentUser(tournamentId) {
  ...
  const active=state.singlePlayerSeason?.activeMatch;
  const activeCleared=Boolean(active&&Number(active.tournamentId||0)===id);
  if(activeCleared)state.singlePlayerSeason.activeMatch=null;
  if(Number(state.activeTournamentId||0)===id)state.activeTournamentId=0;
  state.activeMode='season';

  saveState();
  logger(`[solo-cup] V116 tournament reset tournamentId=${id} activeMatchCleared=${activeCleared}`);
  return {tournamentId:id,success:true,valid:true,reset:true};
}

function extractDiscardItemIds(body) {
  const candidates=[];
  const pushValue=value=>{
    if(value===undefined||value===null)return;
    if(Array.isArray(value)){for(const entry of value)pushValue(entry);return;}
    if(typeof value==='object'){
      pushValue(value.id);
      pushValue(value.itemId);
      return;
    }
    const id=Number(value);
    if(Number.isSafeInteger(id)&&id>0)candidates.push(id);
  };
  pushValue(body?.itemId);
  pushValue(body?.itemIds);
  pushValue(body?.items);
  pushValue(body?.itemData);
  return [...new Set(candidates)];
}

async function handle(req,res,urlPath,requestBody) {
  await waitForMngFullCloudHydration();
  loadState();
  if(['/ut/game/fifa17/user/credits','/ut/game/fifa17/credits','/credits','/ut/game/fifa17/hub','/ut/game/fifa17/user/hub'].includes(String(urlPath||'').toLowerCase())){
    await refreshMngCloudWalletFromServer(`backend:${String(urlPath||'').toLowerCase()}`,750);
  }
  if(!state.__diagHotfixSeen){
    state.__diagHotfixSeen=true;
    logger('[diag] V117 pack odds + dynamic-images recovery active - MNG_V3 local player rolls + GitHub portrait auto-config');
  }
  ...
  if([
    '/ut/game/fifa17/conceptitems',
    '/ut/game/fifa17/conceptitem',
    '/ut/game/fifa17/concept/items',
    '/ut/game/fifa17/dreamsquad/items'
  ].includes(urlPath)&&method==='GET'){
    const response=conceptItems(query);
    logger(`[concept] search returned=${response.count}/${response.total} offset=${response.offset}`);
  ...
  if(urlPath==='/ut/delete/game/fifa17/item'&&method==='POST'){
    const ids=extractDiscardItemIds(body);
    const result=await discardItems(ids);
    logger(`[quicksell] bulk ids=${(result.itemIds||ids).join(',')} value=${Number(result.discardValue)||0} credits=${result.credits}`);
  ...
  if(match&&method==='DELETE'){
    ...
    const owned=state.items.find(item=>Number(item.resourceId)===resourceId&&item.itemState!=='discarded'&&Number(item.pile)!==0);
    const result=owned?await discardItem(owned.id):{items:[],itemData:[],itemIds:[],credits:state.coins,totalCredits:state.coins,coins:state.coins};
    return send(result._status||200,result);
  }
...
// MNG DRAFT V7 - AURORA NATIVE WIRE
  ...
  if(['/ut/game/fifa17/squad/list','/ut/game/fifa17/squad'].includes(urlPath)&&method==='GET'){
    ...
    logger(`[squad-v17] list active=${state.activeSquadId} ids=${list.map(x=>x.id).join(',')} draftMode=${state.activeMode==='draft'?1:0}`);
    return send(200,{
      activeSquadId:state.activeSquadId,
      activeSquad:state.activeSquadId,
      count:list.length,
      squad:list,
      squads:list,
      squadList:list
  ...
  // FIFA's "Create new squad" path. The old backend reused activeSquadId when
  // POST /squad had no id, which could only overwrite Local XI.
  if(urlPath==='/ut/game/fifa17/squad'&&method==='POST'){
    const result=createSquad(body);
    return send(result.status||200,result);
  }

  // Older FUT front ends expose a dedicated SquadClone resource.
  if(urlPath==='/ut/game/fifa17/squad/clone'&&['POST','PUT'].includes(method)){
    const root=body&&(body.squad||body)||{};
    const sourceId=Number(root.sourceSquadId||root.cloneSquadId||root.squadId||root.id||state.activeSquadId)||state.activeSquadId;
    const result=createSquad(body,sourceId);
    return send(result.status||200,result);
  }

  // Explicit rename aliases seen across FUT controllers.
  match=urlPath.match(/^\/ut\/game\/fifa17\/squad\/(\d+)\/(?:name|rename)$/);
  if(match&&['POST','PUT','PATCH'].includes(method)){
    const result=renameSquad(match[1],body);
    return send(result.status||200,result);
  }
  ...
  if(match&&['POST','PUT','PATCH'].includes(method)){
    const root=body&&(body.squad||body)||{};
    const onlyName=
      (root.squadName!==undefined||root.name!==undefined||root.newSquadName!==undefined||root.teamName!==undefined) &&
      !Array.isArray(root.players) && !Array.isArray(root.itemData) &&
      root.formation===undefined && root.manager===undefined &&
      root.managerId===undefined && root.managerItemId===undefined;

    const result=onlyName
      ? renameSquad(match[1],body)
      : saveSquad(body,match[1]);

    return send(result.status||200,result);
  }

  if(['/ut/game/fifa17/squads/active','/ut/game/fifa17/squad/active'].includes(urlPath)&&['POST','PUT'].includes(method)){
    const result=saveSquad(body,state.activeSquadId);
    return send(result.status||200,result);
  }

  if(match&&method==='DELETE'){
    const result=deleteRegularSquad(match[1]);
    return send(result.status||200,result);
  }
  ...
  if(urlPath==='/ut/game/fifa17/leaderboards/options'&&method==='GET')return send(200,seasonLeaderboardOptionsDocument());
  if(/^\/ut\/game\/fifa17\/leaderboards(?:\/|$)/.test(urlPath)&&method==='GET')return send(200,{leaderboardInfo:[],leaderboards:[],entries:[],rank:0,total:0});
  ...
  if(['/ut/game/fifa17/season/user/history','/ut/game/fifa17/season/offline/user/history'].includes(urlPath)&&['GET','POST'].includes(method))return send(200,seasonHistoryDocument(query));
  ...
  if(match&&['POST','PUT'].includes(method))return send(200,updateSeasonData({...body,divisionId:Number(match[1]),seasonId:Number(match[2])}));
  ...
  let deleteTournamentMatch=urlPath.match(/^\/ut\/delete\/game\/fifa17\/tournament\/user\/(\d+)$/i);
  if(deleteTournamentMatch&&['POST','DELETE'].includes(method)){
    const tournamentId=Number(deleteTournamentMatch[1]);
    const document=resetTournamentUser(tournamentId);
    writeTournamentDiag(`[RES] ${method} ${req.url} RESET ${JSON.stringify(document)}`);
    return send(200,document);
  }
  ...
  const lahmLoanSet={
    ...
    description:'Echangez une equipe pour obtenir Lahm 95 en pret pour 10 matchs.',repeatable:false,
  ...
  const riberySet={
    ...
    description:'Echangez quatre equipes pour obtenir Franck Ribery.',repeatable:false,
  ...
  const terryFinalPackAward={
    ...
    packAssetId:4,packAssetID:'4',name:'RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC - John Terry',nameLoc:'RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC - John Terry',
    localizedName:'RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC - John Terry',packName:'RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC - John Terry',packs:'RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC - John Terry',packsAmount:1,
    AWD_PACKS_STRING:'RÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense SBC - John Terry',AWD_PACKS_AMOUNT:1,AWD_PACKS_ASSET_IDS:'4',
    isPack:true,isItem:false,isUntradeable:true
  };
  terryFinalPackAward.name='Recompense SBC - John Terry';
  terryFinalPackAward.nameLoc='Recompense SBC - John Terry';
  terryFinalPackAward.localizedName='Recompense SBC - John Terry';
  terryFinalPackAward.packName='Recompense SBC - John Terry';
  terryFinalPackAward.packs='Recompense SBC - John Terry';
  terryFinalPackAward.AWD_PACKS_STRING='Recompense SBC - John Terry';
...
if(['/ut/game/fifa17/sbs/sets','/ut/game/fifa17/sbc/sets','/sbs/sets'].includes(urlPath)&&method==='GET')return send(200,{
  ...
  const lahmLoanChallenge={
    ...
    description:'Obtenez Lahm 95 en prÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âªt 10 matchs.',formation:'f442',formationId:0,
...
const lahmLoanSbcSquad={
  ...
  const riberyChallengeDefs=[
    {challengeId:960011,id:960011,setId:96001,name:'FC Bayern MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¼nchen',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe avec des joueurs du Bayern.',formation:'f442',formationId:0,packId:203,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
      {type:'TEAM_COUNT',scope:'SQUAD',teamId:21,min:1,value:1,name:'Joueurs du Bayern : minimum 1'},
      {type:'TEAM_RATING',scope:'SQUAD',min:82,value:82,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 82'},
      {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:80,value:80,name:'Collectif : minimum 80'}]},
    {challengeId:960012,id:960012,setId:96001,name:'Les Bleus',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe construite autour de joueurs franÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ais.',formation:'f433',formationId:0,packId:204,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
      {type:'NATION_COUNT',scope:'SQUAD',nationId:18,min:2,value:2,name:'Joueurs franÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ais : minimum 2'},
      {type:'TEAM_RATING',scope:'SQUAD',min:83,value:83,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 83'},
      {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:85,value:85,name:'Collectif : minimum 85'}]},
    {challengeId:960013,id:960013,setId:96001,name:'Bundesliga',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe de Bundesliga avec au moins une carte TOTW.',formation:'f4231',formationId:0,packId:303,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
      {type:'LEAGUE_COUNT',scope:'SQUAD',leagueId:19,min:4,value:4,name:'Joueurs Bundesliga : minimum 4'},
      {type:'SPECIAL_COUNT',scope:'SQUAD',cardType:'totw',min:1,value:1,name:'Joueurs TOTW : minimum 1'},
      {type:'TEAM_RATING',scope:'SQUAD',min:84,value:84,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 84'},
      {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:80,value:80,name:'Collectif : minimum 80'}]},
    {challengeId:960014,id:960014,setId:96001,name:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°quipe 85',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe trÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨s bien notÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©e avec un joueur TOTW pour terminer le dÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©fi RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry.',formation:'f41212',formationId:0,packId:304,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
      {type:'SPECIAL_COUNT',scope:'SQUAD',cardType:'totw',min:1,value:1,name:'Joueurs TOTW : minimum 1'},
      {type:'TEAM_RATING',scope:'SQUAD',min:85,value:85,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 85'},
      {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:65,value:65,name:'Collectif : minimum 65'}]}
  ];
  const riberySafeText=new Map([
    [960011,{name:'FC Bayern Munich',description:'Echangez une equipe avec au moins un joueur du Bayern.',requirements:['Exactement 11 joueurs','Joueurs du Bayern : minimum 1','Note equipe : minimum 82','Collectif : minimum 80']}],
    [960012,{name:'Les Bleus',description:'Echangez une equipe construite autour de joueurs francais.',requirements:['Exactement 11 joueurs','Joueurs francais : minimum 2','Note equipe : minimum 83','Collectif : minimum 85']}],
    [960013,{name:'Bundesliga',description:'Echangez une equipe de Bundesliga avec au moins une carte TOTW.',requirements:['Exactement 11 joueurs','Joueurs Bundesliga : minimum 4','Joueurs TOTW : minimum 1','Note equipe : minimum 84','Collectif : minimum 80']}],
    [960014,{name:'Equipe 85',description:'Echangez une equipe note 85 avec un joueur TOTW pour terminer le defi Ribery.',requirements:['Exactement 11 joueurs','Joueurs TOTW : minimum 1','Note equipe : minimum 85','Collectif : minimum 65']}]
  ]);
  for(const definition of riberyChallengeDefs){
    const safe=riberySafeText.get(definition.challengeId);
    if(!safe)continue;
    definition.name=safe.name;
    definition.description=safe.description;
    definition.requirements.forEach((requirement,index)=>{if(safe.requirements[index])requirement.name=safe.requirements[index];});
  }
  const riberyChallenges=riberyChallengeDefs.map(def=>{
    const progress=riberyChallengeState(def.challengeId);
    const packAward={type:'pack',value:def.packId,halId:0,count:1,assetId:3,packId:def.packId,name:PACKS[def.packId]?.name||'Pack rÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense'};
  ...
  if(match){
    ...
    if(method==='PUT'){
      ...
      if(allDone&&!state.sbcRiberyCompleted&&setCloudReward){
        finalRewardPack={id:Number(state.nextRewardPackId++),packId:296001,source:'SBC_RIBERY_FINAL',setId:96001,playerName:'Franck RibÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©ry',rewardResourceId:RIBERY_SBC_RESOURCE_ID,cloudResourceIds:setCloudReward.resourceIds||[],cloudTransactionId:setCloudReward.transactionId};
  ...
  const terryChallengeDefs=[
    {challengeId:960111,id:960111,setId:96011,name:'Chelsea',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe avec des joueurs de Chelsea.',formation:'f442',formationId:0,packId:304,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
      {type:'TEAM_COUNT',scope:'SQUAD',teamId:5,min:2,value:2,name:'Joueurs de Chelsea : minimum 2'},
      {type:'TEAM_RATING',scope:'SQUAD',min:82,value:82,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 82'},
      {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:80,value:80,name:'Collectif : minimum 80'}]},
    {challengeId:960112,id:960112,setId:96011,name:'Three Lions',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe construite autour de joueurs anglais.',formation:'f433',formationId:0,packId:305,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
      {type:'NATION_COUNT',scope:'SQUAD',nationId:14,min:4,value:4,name:'Joueurs anglais : minimum 4'},
      {type:'TEAM_RATING',scope:'SQUAD',min:83,value:83,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 83'},
      {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:85,value:85,name:'Collectif : minimum 85'}]},
    {challengeId:960113,id:960113,setId:96011,name:'Premier League',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe de Premier League avec au moins une carte TOTW.',formation:'f4231',formationId:0,packId:305,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
    ...
    {challengeId:960114,id:960114,setId:96011,name:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°quipe 85',description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°changez une ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe trÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨s bien notÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©e pour terminer le Flashback de Terry.',formation:'f41212',formationId:0,packId:308,requirements:[
      {type:'PLAYER_COUNT',scope:'SQUAD',count:11,min:11,value:11,name:'Exactement 11 joueurs'},
      {type:'TEAM_RATING',scope:'SQUAD',min:85,value:85,name:'Note ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©quipe : minimum 85'},
      {type:'TEAM_CHEMISTRY',scope:'SQUAD',min:70,value:70,name:'Collectif : minimum 70'}]}
  ];
  const terryCleanText={
    960111:{name:'Chelsea',description:'Echangez une equipe avec des joueurs de Chelsea.',rating:'Note equipe : minimum 82'},
    960112:{name:'Three Lions',description:'Echangez une equipe construite autour de joueurs anglais.',rating:'Note equipe : minimum 83'},
    960113:{name:'Premier League',description:'Echangez une equipe de Premier League avec au moins une carte TOTW.',rating:'Note equipe : minimum 84'},
    960114:{name:'Equipe 85',description:'Echangez une equipe tres bien notee pour terminer le Flashback de Terry.',rating:'Note equipe : minimum 85'}
  };
  for(const definition of terryChallengeDefs){
    const clean=terryCleanText[Number(definition.challengeId)];
    if(!clean)continue;
    definition.name=clean.name;
    definition.description=clean.description;
    const ratingRequirement=definition.requirements.find(requirement=>requirement.type==='TEAM_RATING');
    if(ratingRequirement)ratingRequirement.name=clean.rating;
  }
  ...
  if(match){
    ...
    if(method==='PUT'){
      ...
      if(finalRewardPack){
        ...
        response.groupCompleted=true;response.setCompleted=true;response.success=true;response.message='Groupe terminÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© !';
  ...
  const communityChallengeDocuments=COMMUNITY_SBC_DEFS.map(def=>{
    ...
    const challengeAward={
      ...
      packId:challengePackId,name:challengePack?.name||'Pack rÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©compense'
    };
    ...
    const challenge={
      ...
      description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°quipe 83 avec 2 BrÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©siliens, 3 joueurs de Serie A, 1 joueur TOTW et collectif minimum 70.',
    ...
    challenge.description='Equipe 83 avec 2 Bresiliens, 3 joueurs de Serie A, 1 joueur TOTW et collectif minimum 70.';
  ...
  if(match){
    ...
    if(method==='PUT'){
      ...
      return send(200,{
        completed:true,isCompleted:true,groupCompleted:true,setCompleted:true,success:true,message:'Groupe terminÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© !',
  ...
  const benYedderChallenge={
    ...
    description:'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°quipe 82 avec au moins 2 FranÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ais, 3 joueurs de Liga et collectif minimum 75.',
  ...
  benYedderChallenge.description='Equipe 82 avec au moins 2 Francais, 3 joueurs de Liga et collectif minimum 75.';
  benYedderChallenge.requirements[1].name='Joueurs francais : minimum 2';
  ...
  if(urlPath==='/ut/game/fifa17/match/ready'&&['GET','POST','PUT'].includes(method)){
    if(method==='PUT'&&isOfflineMatchReadyRequest(body)){
      const result=offlineMatchReadyDocument(body);
      return send(result._status||200,result);
    }
    return send(200,{valid:true,success:true,ready:true,
  ...
  if(urlPath==='/ut/game/fifa17/match/reset'&&['GET','POST','PUT','DELETE'].includes(method)){
    ...
    // V114: leave cloud authorization state intact, but never let a cloud-only
    // run masquerade as a native tournament-in-progress after bootstrap/reset.
    const tournamentProgress=ensureTournamentProgressState();
    let hiddenCloudOnlyRuns=0;
    for(const entry of Object.values(tournamentProgress)){
      if(entry && typeof entry==='object' && String(entry.cloudRunToken||'') && !tournamentProgressIsResumable(entry)){
        entry.nativeAdvertised=false;
        hiddenCloudOnlyRuns++;
      }
    }
    if(hiddenCloudOnlyRuns){
      logger(`[solo-cup] V114 match/reset hid ${hiddenCloudOnlyRuns} cloud-only tournament run(s) from native user/list`);
    }
  ...
  if(match&&['GET','POST','PUT'].includes(method)){
    ...
    return send(200,{valid:true,success:true,ready:true,matchId:Number(match[1]),opponentPersonaId:0,items:[]});
  }
  ...
  // V113: FIFA's second-stage MatchReady request is:
  // PUT /ut/game/fifa17/match {"items":[11 starter ids]}
  // This must NOT create/re-authorize a second match session.
  if(urlPath==='/ut/game/fifa17/match'&&method==='PUT'&&isOfflineMatchReadyRequest(body)){
    const result=offlineMatchReadyDocument(body);
    return send(result._status||200,result);
  }
  ...
  if(urlPath==='/fut/packs/dreamsquad/dreamsquadpacklist.json'){
    const response=dreamSquadPackList();
    logger(`[concept] selector packs=${response.packList.length}`);
    return send(200,response);
  }
  return false;
}

module.exports={init,handle,openStorePack,startSecureMatch,finishSecureMatch,cloudMarketSearch,cloudListOwnedItem,cloudBuyMarketListing,cloudTradePile,cloudTradePileCounts,cloudTradeStatus,cloudWatchList,cloudHubDocument,cloudRelistExpired,cloudClearMarketListing,relistExpiredListings,clearFinishedListings,squadDocument,squadList,hubDocument,creditsDocument,homeWalletDocument,homeRecordDocument,settingsDocument,pileSizeDocument,seasonListDocument,seasonUserDocument,seasonHistoryDocument,
  clubStats,packCatalogue,openPack,filteredClub,purchasedResponse,marketSearch,buyMarketListing,updateItems,storeDescriptionsXml,dreamSquadPackList,
  getState:()=>state,getCatalog:()=>catalog,getIdentity,setIdentity,getTotwIdentity,
  syncMngCloudWallet,queueMngCloudWalletSync,refreshMngCloudWalletFromServer,syncMngCloudClub,queueMngCloudClubSync,
  syncMngFullCloudSave,queueMngFullSaveSync,waitForMngFullCloudHydration,
  getFullCloudStatus:()=>({supported:MNG_FULL_SAVE_SUPPORTED,ready:MNG_FULL_SAVE_READY,revision:MNG_FULL_SAVE_REVISION,conflict:MNG_FULL_SAVE_CONFLICT,endpoint:MNG_FULL_SAVE_API_BASE_URL}),
  totwUserListDocument,totwPublicUserDocument,totwSquadDocument,totwSessionActive};
