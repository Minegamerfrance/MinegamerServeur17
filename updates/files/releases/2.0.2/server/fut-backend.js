...
async function cloudListOwnedItem(body={}) {
  ...
  if(!item){
    const payload=await mngCloudMarketRequest('/api/market/list',{method:'POST',body:{...body,itemId,resourceId:Number(body?.itemData?.resourceId)||0}});
    if(!payload.ok)return {status:payload.status,error:payload.error,reason:payload.error};
  ...
  if(!await forceMngCloudClubSync('before-market-list'))return {status:503,error:'CLUB_SYNC_FAILED',reason:'CLUB_SYNC_FAILED'};
  const payload=await mngCloudMarketRequest('/api/market/list',{method:'POST',body:{...body,itemId,resourceId:Number(item.resourceId)}});
  if(!payload.ok)return {status:payload.status,error:payload.error,reason:payload.error};
