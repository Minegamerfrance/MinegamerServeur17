...
function Start-OrWait-MngFifa {
    ...
    if($env:MNG_FUT_LAUNCHER_OWNS_GAME -eq '1'){
        $readyDeadline=[DateTime]::UtcNow.AddSeconds(20)
        while([DateTime]::UtcNow -lt $readyDeadline){
            if((Test-MngLocalPort 8000) -and (Test-MngLocalPort 5139)){break}
            Start-Sleep -Milliseconds 200
        }

        if(-not((Test-MngLocalPort 8000) -and (Test-MngLocalPort 5139))){
            throw 'MNG local server ports 8000/5139 did not become ready before launcher handoff.'
        }
