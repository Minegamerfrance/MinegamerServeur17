...
function tradePileDocument() {
  ...
  const auctionInfo=(state.listings||[]).filter(entry=>entry.tradeState!=='closed').map(listing=>({
    ...listing,id:Number(listing.tradeId),tradeId:Number(listing.tradeId),tradeState:listing.tradeState==='inactive'?'invalid':listing.tradeState,itemData:listing.itemData,
...
function tradeStatusDocument(query) {
  ...
  return {auctionInfo:listings.map(entry=>({...entry,id:Number(entry.tradeId),tradeId:Number(entry.tradeId),tradeState:entry.tradeState==='inactive'?'invalid':entry.tradeState,tradeOwner:true})),duplicateItemIdList:[],total:listings.length,credits:state.coins,totalCredits:state.coins};
}
