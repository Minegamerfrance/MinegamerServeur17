...
function tradePileDocument() {
  ...
  const auctionInfo=(state.listings||[]).filter(entry=>entry.tradeState!=='closed').map(listing=>({
    ...listing,id:Number(listing.tradeId),tradeId:listing.tradeState==='inactive'?0:Number(listing.tradeId),itemData:listing.itemData,
...
function tradeStatusDocument(query) {
  ...
  return {auctionInfo:listings.map(entry=>({...entry,id:Number(entry.tradeId),tradeId:entry.tradeState==='inactive'?0:Number(entry.tradeId),tradeOwner:true})),duplicateItemIdList:[],total:listings.length,credits:state.coins,totalCredits:state.coins};
}
